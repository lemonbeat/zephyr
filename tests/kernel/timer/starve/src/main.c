...
static const char *tag(void)
{
	...
	snprintf(buf, sizeof(buf), "[%6u.%03u] %u: ", now / MSEC_PER_SEC,
		 now % MSEC_PER_SEC, iters);
	return buf;
}
...
static void test_starve(void)
{
	...
	TC_PRINT("There are %u cycles per tick (%u Hz ticks)\n",
		 k_ticks_to_cyc_ceil32(1U), k_ms_to_ticks_ceil32(MSEC_PER_SEC));
	...
	while (true) {
		...
		zassert_true(ticks_diff > 0, "%sTicks went backwards by %d\n",
			     tag(), -(int32_t)ticks_diff);
		last_ticks = ticks;

		uint32_t na_capture = na;

		zassert_equal(na_capture, 0, "%sTimer alarm fired: %u\n",
			      na_capture);
