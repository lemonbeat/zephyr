...
typedef int (*pool_block_get_func_t)(struct k_mem_block *, struct k_mem_pool *,
				     int, int32_t);
typedef int (*pool_move_block_func_t)(struct k_mem_block *,
				      struct k_mem_pool *);
...
static struct TEST_CASE get_set[] = { { &block_list[0], &POOL_ID, 0, 0, 0 },
...
#else
				      { &block_list[8], &POOL_ID, 2048, 0,
					-ENOMEM },
#endif
				      { &block_list[9], &POOL_ID, 4096, 0,
					-ENOMEM } };
 ...
 */
static int pool_block_get_func(struct k_mem_block *block,
			       struct k_mem_pool *pool, int size,
			       int32_t unused)
{
	ARG_UNUSED(unused);
 ...
 */
static int pool_block_get_wt_func(struct k_mem_block *block,
				  struct k_mem_pool *pool, int size,
				  int32_t timeout)
{
				...
				struct TEST_CASE *tests, int n_tests)
{
	...
	for (i = 0; i < n_tests; i++) {
		rv = func(tests[i].block, tests[i].pool_id, tests[i].size,
			  tests[i].timeout);
		zassert_equal(rv, tests[i].rcode,
			      "%s() expected %d, got %d\n"
			      "size: %d, timeout: %d\n",
			      string, tests[i].rcode, rv, tests[i].size,
			      tests[i].timeout);
	}
}
...
static void test_pool_block_get_wait(void)
{
	...
	switch (evidence) {
	case 0:
		zassert_true(evidence == 0,
			     "k_mem_pool_alloc(128) did not block!");
	case 1:
		break;
	case 2:
	default:
...
K_THREAD_DEFINE(t_alternate, STACKSIZE, alternate_task, NULL, NULL, NULL, 6, 0,
		0);

K_THREAD_DEFINE(t_helper, STACKSIZE, helper_task, NULL, NULL, NULL, 7, 0, 0);

void test_main(void)
{
	ztest_test_suite(mempool, ztest_unit_test(test_pool_block_get),
