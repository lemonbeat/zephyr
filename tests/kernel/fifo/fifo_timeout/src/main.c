...
struct timeout_order_data timeout_order_data[] = {
	{ 0, &fifo_timeout[0], 20, 2, 0 }, { 0, &fifo_timeout[0], 40, 4, 1 },
	{ 0, &fifo_timeout[0], 0, 0, 2 },  { 0, &fifo_timeout[0], 10, 1, 3 },
	{ 0, &fifo_timeout[0], 30, 3, 4 },
};

struct timeout_order_data timeout_order_data_mult_fifo[] = {
	{ 0, &fifo_timeout[1], 0, 0, 0 },  { 0, &fifo_timeout[0], 30, 3, 1 },
	{ 0, &fifo_timeout[0], 50, 5, 2 }, { 0, &fifo_timeout[1], 80, 8, 3 },
	{ 0, &fifo_timeout[1], 70, 7, 4 }, { 0, &fifo_timeout[0], 10, 1, 5 },
	{ 0, &fifo_timeout[0], 60, 6, 6 }, { 0, &fifo_timeout[0], 20, 2, 7 },
					 ...
					 int test_data_size)
{
	...
	for (ii = 0; ii < test_data_size; ii++) {
		tid[ii] = k_thread_create(&ttdata[ii], ttstack[ii], TSTACK_SIZE,
					  ...
					  FIFO_THREAD_PRIO, K_INHERIT_PERMS,
					  K_NO_WAIT);
	}
					  ...
					  int test_data_size)
{
	...
	tid[ii] = k_thread_create(&ttdata[ii], ttstack[ii], TSTACK_SIZE,
				  test_thread_pend_and_timeout, &test_data[ii],
...
static void test_thread_timeout_reply_values(void *p1, void *p2, void *p3)
{
	...
	reply_packet->reply = !!k_fifo_get(&fifo_timeout[0], K_NO_WAIT);
...
static void test_thread_timeout_reply_values_wfe(void *p1, void *p2, void *p3)
{
	...
	reply_packet->reply = !!k_fifo_get(&fifo_timeout[0], K_FOREVER);
...
static void test_timeout_fifo_thread(void)
{
	...
	tid[0] = k_thread_create(&ttdata[0], ttstack[0], TSTACK_SIZE,
				 ...
				 &reply_packet, NULL, NULL, FIFO_THREAD_PRIO,
				 K_INHERIT_PERMS, K_NO_WAIT);
