...
void test_priority_preemptible(void)
{
	...
	tid = k_thread_create(&tdata, tstack, STACK_SIZE, thread_entry, NULL,
			      NULL, NULL, spawn_prio, 0, K_NO_WAIT);
...
void test_bad_priorities(void)
{
	struct prio_test {
		...
		void *entry;
		bool result;
	} testcases[] = { { K_IDLE_PRIO, idle, true },
			  { K_IDLE_PRIO, NULL, false },
			  { K_HIGHEST_APPLICATION_THREAD_PRIO - 1, NULL,
			    false },
