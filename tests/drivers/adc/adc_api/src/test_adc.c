...
#define ADC_2ND_CHANNEL_INPUT NRF_ADC_CONFIG_INPUT_3
	...
	defined(CONFIG_BOARD_BL652_DVK) || defined(CONFIG_BOARD_BL653_DVK) ||  \
	defined(CONFIG_BOARD_BL654_DVK) || defined(CONFIG_BOARD_DEGU_EVK) ||   \
	defined(CONFIG_BOARD_ADAFRUIT_FEATHER_NRF52840) ||                     \
	defined(CONFIG_BOARD_RUUVI_RUUVITAG) || defined(CONFIG_BOARD_BT510) || \
...
static void check_samples(int expected_count)
{
	...
	for (i = 0; i < BUFFER_SIZE; i++) {
		...
		if (i < expected_count) {
			...
			zassert_equal(0, sample_value, "[%u] should be zero",
				      i);
		}
	}
	TC_PRINT("\n");
