			 ...
			 const uint8_t *comp_buffer)
{
	...
	LOG_INF("Start full read. Master: %s, address: 0x%x", i2c->name, addr);
	...
	if (memcmp(i2c_buffer, comp_buffer, TEST_DATA_SIZE)) {
		to_display_format(i2c_buffer, TEST_DATA_SIZE, buffer_print_i2c);
		to_display_format(comp_buffer, TEST_DATA_SIZE,
				  buffer_print_eeprom);
		LOG_ERR("Buffer contents are different: %s", buffer_print_i2c);
			    ...
			    const uint8_t *comp_buffer, unsigned int offset)
{
	...
	ret = i2c_burst_read(i2c, addr, offset, i2c_buffer,
			    ...
			    unsigned int offset)
{
	...
	LOG_INF("Start program. Master: %s, address: 0x%x, off=%d", i2c->name,
		addr, offset);
	...
	ret = i2c_burst_write(i2c, addr, offset, i2c_buffer,
			      TEST_DATA_SIZE - offset);
...
void test_eeprom_slave(void)
{
	...
	LOG_INF("Found EP0 %s on I2C Master device %s at addr %02x", label_0,
		DT_BUS_LABEL(NODE_EP0), addr_0);
	...
	LOG_INF("Found EP1 %s on I2C Master device %s at addr %02x", label_1,
		DT_BUS_LABEL(NODE_EP1), addr_1);
	...
	zassert_equal(ret, 0, "Full I2C read from EP0 failed");
	ret = run_full_read(i2c_0, addr_1, eeprom_1_data);
	zassert_equal(ret, 0, "Full I2C read from EP1 failed");

	for (offset = 0; offset < TEST_DATA_SIZE - 1; ++offset) {
		zassert_equal(0,
			      run_partial_read(i2c_1, addr_0, eeprom_0_data,
					       offset),
			      "Partial I2C read EP0 failed");
		zassert_equal(0,
			      run_partial_read(i2c_0, addr_1, eeprom_1_data,
					       offset),
