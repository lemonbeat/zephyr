...
#define TEST_SIM_FLASH_END (TEST_SIM_FLASH_SIZE + FLASH_SIMULATOR_BASE_OFFSET)

#define PATTERN8TO32BIT(pat)                                                   \
	(((((((0xff & pat) << 8) | (0xff & pat)) << 8) | (0xff & pat)) << 8) | \
	 (0xff & pat))
...
static void test_read(void)
{
	...
	rc = flash_read(flash_dev, FLASH_SIMULATOR_BASE_OFFSET, test_read_buf,
			sizeof(test_read_buf));
	zassert_equal(0, rc, "flash_read should succeed");

	for (i = 0; i < sizeof(test_read_buf); i++) {
		zassert_equal(FLASH_SIMULATOR_ERASE_VALUE, test_read_buf[i],
...
static void test_write_read(void)
{
	...
	for (off = 0; off < TEST_SIM_FLASH_SIZE; off += 4) {
		rc = flash_write_protection_set(flash_dev, false);
		zassert_equal(0, rc, NULL);
		rc = flash_write(flash_dev, FLASH_SIMULATOR_BASE_OFFSET + off,
				 &val32, sizeof(val32));
		zassert_equal(0, rc,
	...
	for (off = 0; off < TEST_SIM_FLASH_SIZE; off += 4) {
		rc = flash_read(flash_dev, FLASH_SIMULATOR_BASE_OFFSET + off,
...
static void test_erase(void)
{
	...
	test_check_pattern32(FLASH_SIMULATOR_BASE_OFFSET +
				     FLASH_SIMULATOR_ERASE_UNIT,
			     pattern32_flat, FLASH_SIMULATOR_ERASE_UNIT);
	...
	test_check_pattern32(FLASH_SIMULATOR_BASE_OFFSET +
			     ...
			     pattern32_inc, FLASH_SIMULATOR_ERASE_UNIT * 2);
}
...
static void test_access(void)
{
	...
	rc = flash_write(flash_dev, FLASH_SIMULATOR_BASE_OFFSET, data,
			 sizeof(data));
...
static void test_out_of_bounds(void)
{
	...
	rc = flash_write(flash_dev, FLASH_SIMULATOR_BASE_OFFSET - 4, data, 4);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_write(flash_dev, FLASH_SIMULATOR_BASE_OFFSET - 4, data, 8);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_write(flash_dev, TEST_SIM_FLASH_END, data, 4);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_write(flash_dev, TEST_SIM_FLASH_END - 4, data, 8);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);
	...
	rc = flash_erase(flash_dev,
			 TEST_SIM_FLASH_END - FLASH_SIMULATOR_ERASE_UNIT,
			 FLASH_SIMULATOR_ERASE_UNIT * 2);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_read(flash_dev, FLASH_SIMULATOR_BASE_OFFSET - 4, data, 4);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_read(flash_dev, FLASH_SIMULATOR_BASE_OFFSET - 4, data, 8);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_read(flash_dev, TEST_SIM_FLASH_END, data, 4);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);
...
static void test_align(void)
{
	...
	rc = flash_read(flash_dev, FLASH_SIMULATOR_BASE_OFFSET + 1, data, 4);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_write(flash_dev, FLASH_SIMULATOR_BASE_OFFSET + 1, data, 4);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);

	rc = flash_write(flash_dev, FLASH_SIMULATOR_BASE_OFFSET, data, 3);
	zassert_equal(-EINVAL, rc, "Unexpected error code (%d)", rc);
...
static void test_get_erase_value(void)
{
	...
	zassert_equal(fp->erase_value, FLASH_SIMULATOR_ERASE_VALUE,
		      "Expected erase value %x", FLASH_SIMULATOR_ERASE_VALUE);
}

void test_main(void)
{
	ztest_test_suite(flash_sim_api, ztest_unit_test(test_init),
