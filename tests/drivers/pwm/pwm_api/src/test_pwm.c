...
static int test_task(uint32_t port, uint32_t period, uint32_t pulse,
		     uint8_t unit)
{
...
void test_pwm_usec(void)
{
	...
	zassert_true(test_task(DEFAULT_PWM_PORT, DEFAULT_PERIOD_USEC,
			       DEFAULT_PULSE_USEC, UNIT_USECS) == TC_PASS,
		     NULL);
	k_sleep(K_MSEC(1000));

	/* Period : Pulse (2000 : 2000), unit (usec). Voltage : 3.3V */
	zassert_true(test_task(DEFAULT_PWM_PORT, DEFAULT_PERIOD_USEC,
			       DEFAULT_PERIOD_USEC, UNIT_USECS) == TC_PASS,
		     NULL);
	k_sleep(K_MSEC(1000));

	/* Period : Pulse (2000 : 0), unit (usec). Voltage : 0V */
	zassert_true(test_task(DEFAULT_PWM_PORT, DEFAULT_PERIOD_USEC, 0,
			       UNIT_USECS) == TC_PASS,
		     NULL);
	k_sleep(K_MSEC(1000));
}
...
void test_pwm_nsec(void)
{
	...
	zassert_true(test_task(DEFAULT_PWM_PORT, DEFAULT_PERIOD_NSEC, 0,
			       UNIT_NSECS) == TC_PASS,
		     NULL);
	k_sleep(K_MSEC(1000));
}
...
void test_pwm_cycle(void)
{
	...
	zassert_true(test_task(DEFAULT_PWM_PORT, DEFAULT_PERIOD_CYCLE, 0,
			       UNIT_CYCLES) == TC_PASS,
		     NULL);
}
