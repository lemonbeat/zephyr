...
/* Nordic RTC1 is used for the system clock */
	 ...
	 * handles it.
	 */
	LABELS_FOR_DT_COMPAT(arm_cmsdk_timer) LABELS_FOR_DT_COMPAT(
		arm_cmsdk_dtimer) LABELS_FOR_DT_COMPAT(microchip_xec_timer)
		LABELS_FOR_DT_COMPAT(nxp_imx_epit) LABELS_FOR_DT_COMPAT(
			nxp_imx_gpt)
			  ...
			  uint32_t counter, void *user_data)
{
	...
	if (counter_is_counting_up(dev)) {
		diff = (now < counter) ? (now + top - counter) :
					       (now - counter);
	} else {
		diff = (now > counter) ? (counter + top - now) :
					       (counter - now);
	}
...
void test_all_channels_instance(const char *dev_name)
{
	...
	for (int i = 0; i < n; i++) {
		...
		if ((err == 0) && !limit_reached) {
			...
			zassert_equal(0, 1,
				      "%s: Unexpected error on setting alarm",
				      dev_name);
		}
	}
