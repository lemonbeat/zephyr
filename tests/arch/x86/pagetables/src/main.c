 ...
 */
#define FLAGS_MASK \
	(MMU_P | MMU_RW | MMU_US | MMU_PWT | MMU_PCD | MMU_G | MMU_PS | MMU_XD)
...
void test_ram_perms(void)
{
	...
	for (pos = VM_BASE; pos < VM_LIMIT; pos += CONFIG_MMU_PAGE_SIZE) {
		...
		zassert_true((entry & MMU_P) != 0, "non-present RAM entry");
		...
		zassert_equal(flags, expected, "bad flags " PRI_ENTRY " at %p",
			      flags, pos);
	}
}
...
void test_main(void)
{
	ztest_test_suite(x86_pagetables, ztest_unit_test(test_ram_perms),
