...
void test_main(void)
{
	...
	uint32_t main_stack_base =
		(uint32_t)Z_THREAD_STACK_BUFFER(z_main_stack);
	uint32_t main_stack_top =
		(uint32_t)(Z_THREAD_STACK_BUFFER(z_main_stack) +
			   K_THREAD_STACK_SIZEOF(z_main_stack));

	__ASSERT((psp >= main_stack_base) && (psp <= main_stack_top),
		 "PSP out of bounds: 0x%x (0x%x - 0x%x)", psp, main_stack_base,
		 main_stack_top);

#if defined(CONFIG_FPU)
	__ASSERT(__get_FPSCR() == 0, "FPSCR not zero (0x%x)", __get_FPSCR());
#endif

#if defined(CONFIG_BUILTIN_STACK_GUARD)
	uint32_t psplim = (uint32_t)__get_PSPLIM();
	__ASSERT((psplim == main_stack_base),
		 "PSPLIM not set to main stack base: (0x%x)", psplim);
#endif
	...
	__ASSERT(i >= 0, "No available IRQ line to use in the test\n");
