				...
				const _callee_saved_t *dst)
{
	...
	zassert_true(
		(src->v1 == dst->v1) && (src->v2 == dst->v2) &&
			(src->v3 == dst->v3) && (src->v4 == dst->v4) &&
			(src->v5 == dst->v5) && (src->v6 == dst->v6) &&
		...
		src->v1, src->v2, src->v3, src->v4, src->v5, src->v6, src->v7,
		src->v8, dst->v1, dst->v2, dst->v3, dst->v4, dst->v5, dst->v6,
		dst->v7, dst->v8);
}
...
static void
load_fp_callee_saved_regs(const volatile struct _preempt_float *regs)
{
				   ...
				   const struct _preempt_float *dst)
{
	...
	zassert_true(
		(src->s16 == dst->s16) && (src->s17 == dst->s17) &&
			(src->s18 == dst->s18) && (src->s19 == dst->s19) &&
			(src->s20 == dst->s20) && (src->s21 == dst->s21) &&
			(src->s22 == dst->s22) && (src->s23 == dst->s23) &&
			(src->s24 == dst->s24) && (src->s25 == dst->s25) &&
			(src->s26 == dst->s26) && (src->s27 == dst->s27) &&
			(src->s28 == dst->s28) && (src->s29 == dst->s29) &&
			(src->s30 == dst->s30) && (src->s31 == dst->s31),
		" got: 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x"
		" 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x\n"
		" expected:  0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x"
		" 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x 0x%0x\n",
		src->s16, src->s17, src->s18, src->s19, src->s20, src->s21,
		src->s22, src->s23, src->s24, src->s25, src->s26, src->s27,
		src->s28, src->s29, src->s30, src->s31, dst->s16, dst->s17,
		dst->s18, dst->s19, dst->s20, dst->s21, dst->s22, dst->s23,
		dst->s24, dst->s25, dst->s26, dst->s27, dst->s28, dst->s29,
		dst->s30, dst->s31);
}
...
static void alt_thread_entry(void)
{
	...
	zassert_true(
		p_ztest_thread->arch.swap_return_value == -EAGAIN,
	 ...
	 */
	memset(&ztest_thread_callee_saved_regs_container, 0,
	       sizeof(_callee_saved_t));
	...
	zassert_true(
		(p_ztest_thread->arch.mode & CONTROL_FPCA_Msk) != 0,
	 ...
	 */
	verify_fp_callee_saved((const struct _preempt_float *)&p_ztest_thread
				       ->arch.preempt_float,
	 ...
	 * later, that it is populated properly.
	 */
	memset(&ztest_thread_fp_callee_saved_regs, 0,
	       sizeof(ztest_thread_fp_callee_saved_regs));
...
void test_arm_thread_swap(void)
{
	...
	k_thread_create(&alt_thread, alt_thread_stack,
			K_THREAD_STACK_SIZEOF(alt_thread_stack),
			(k_thread_entry_t)alt_thread_entry, NULL, NULL, NULL,
	 ...
	 */
	verify_callee_saved(&ztest_thread_callee_saved_regs_container,
	 ...
	 * container of the thread.
	 */
	verify_fp_callee_saved(&ztest_thread_fp_callee_saved_regs,
