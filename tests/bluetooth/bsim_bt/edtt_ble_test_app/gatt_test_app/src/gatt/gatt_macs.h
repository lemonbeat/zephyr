...
#define BT_GATT_H_ATTRIBUTE(_uuid, _perm, _read, _write, _value, _handle)     \
	{                                                                     \
		.uuid = _uuid, .perm = _perm, .read = _read, .write = _write, \
		.user_data = _value, .handle = _handle                        \
	}
...
#define BT_GATT_H_CHARACTERISTIC(_uuid, _props, _perm, _read, _write, _value, \
				 _handle)                                     \
	BT_GATT_H_ATTRIBUTE(BT_UUID_GATT_CHRC, BT_GATT_PERM_READ,             \
			    bt_gatt_attr_read_chrc, NULL,                     \
			    (&(struct bt_gatt_chrc){ .uuid = _uuid,           \
						     .properties = _props }), \
			    _handle),                                         \
		BT_GATT_H_ATTRIBUTE(_uuid, _perm, _read, _write, _value,      \
				    _handle + 1)
...
#define BT_GATT_H_PRIMARY_SERVICE(_service, _handle)                   \
	BT_GATT_H_ATTRIBUTE(BT_UUID_GATT_PRIMARY, BT_GATT_PERM_READ,   \
...
#define BT_GATT_H_SECONDARY_SERVICE(_service, _handle)                 \
	BT_GATT_H_ATTRIBUTE(BT_UUID_GATT_SECONDARY, BT_GATT_PERM_READ, \
			    bt_gatt_attr_read_service, NULL, _service, \
			    _handle)
...
#define BT_GATT_H_INCLUDE_SERVICE(_service_incl, _handle)                    \
	BT_GATT_H_ATTRIBUTE(BT_UUID_GATT_INCLUDE, BT_GATT_PERM_READ,         \
...
#define BT_GATT_H_MANAGED(_ccc, _perm, _handle)                             \
	BT_GATT_H_ATTRIBUTE(BT_UUID_GATT_CCC, _perm, bt_gatt_attr_read_ccc, \
			    bt_gatt_attr_write_ccc, _ccc, _handle)
 ...
 */
#define BT_GATT_H_CCC(_cfg, _cfg_changed, _handle)                        \
	BT_GATT_H_MANAGED((&(struct _bt_gatt_ccc)BT_GATT_CCC_INITIALIZER( \
				  _cfg_changed, NULL, NULL)),             \
...
#define BT_GATT_H_CEP(_value, _handle)                                    \
	BT_GATT_H_DESCRIPTOR(BT_UUID_GATT_CEP, BT_GATT_PERM_READ,         \
...
#define BT_GATT_H_CUD(_value, _perm, _handle)                                \
	BT_GATT_H_DESCRIPTOR(BT_UUID_GATT_CUD, _perm, bt_gatt_attr_read_cud, \
			     NULL, (void *)_value, _handle)
...
#define BT_GATT_H_CPF(_value, _handle)                                    \
	BT_GATT_H_DESCRIPTOR(BT_UUID_GATT_CPF, BT_GATT_PERM_READ,         \
