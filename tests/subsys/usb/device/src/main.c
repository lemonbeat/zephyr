...
static void status_cb(struct usb_cfg_data *cfg, enum usb_dc_status_code status,
		      const uint8_t *param)
{
	ARG_UNUSED(cfg);
	ARG_UNUSED(status);
...
static struct usb_ep_cfg_data device_ep[] = {
	{ .ep_cb = bulk_in, .ep_addr = ENDP_BULK_IN },
};
...
static void test_usb_dc_api(void)
{
	...
	zassert_equal(usb_dc_ep_mps(0x0), 64, "usb_dc_ep_mps(0x00) failed");
	zassert_equal(usb_dc_ep_mps(0x80), 64, "usb_dc_ep_mps(0x80) failed");
