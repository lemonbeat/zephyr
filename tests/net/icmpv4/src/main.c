...
static const unsigned char icmpv4_echo_req[] = {
	/* IPv4 Header */
	0x45, 0x00, 0x00, 0x54, 0xea, 0x8c, 0x40, 0x00, 0x40, 0x01, 0xcc, 0x18,
	0xc0, 0x00, 0x02, 0x02, 0xc0, 0x00, 0x02, 0x01,
	/* ICMP Header (Echo Request) */
	0x08, 0x00, 0xe3, 0x7c, 0x10, 0x63, 0x00, 0x01,
	/* Payload */
	0xb8, 0xa4, 0x8c, 0x5d, 0x00, 0x00, 0x00, 0x00, 0xfc, 0x49, 0x04, 0x00,
...
static const unsigned char icmpv4_echo_rep[] = {
	/* IPv4 Header */
	0x45, 0x00, 0x00, 0x20, 0x75, 0xac, 0x00, 0x00, 0x40, 0x01, 0x81, 0x2d,
	0xc0, 0x00, 0x02, 0x02, 0xc0, 0x00, 0x02, 0x01,
	/* ICMP Header (Echo Reply)*/
	0x00, 0x00, 0x91, 0x12, 0x16, 0x50, 0x00, 0x00, 0x01, 0xfd, 0x56, 0xa0
};

static const unsigned char icmpv4_echo_req_opt[] = {
	/* IPv4 Header */
	0x4e, 0x00, 0x00, 0x78, 0xe1, 0x4b, 0x40, 0x00, 0x40, 0x01, 0x9a, 0x83,
	0xc0, 0x00, 0x02, 0x02, 0xc0, 0x00, 0x02, 0x01,
	/* IPv4 Header Options (Timestamp) */
	0x44, 0x24, 0x0d, 0x01, 0xc0, 0x00, 0x02, 0x02, 0x02, 0x4d, 0x1c, 0x3d,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	/* ICMP Header (Echo Request) */
	0x08, 0x00, 0x35, 0xbf, 0x5d, 0xe7, 0x00, 0x01, 0xcf, 0xe7, 0x8d, 0x5d,
	0x00, 0x00, 0x00, 0x00,
	/* Payload */
	0x3a, 0x40, 0x0e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x11, 0x12, 0x13,
...
static const unsigned char icmpv4_echo_req_opt_bad[] = {
	/* IPv4 Header */
	0x46, 0x00, 0x00, 0xa0, 0xf8, 0x6c, 0x00, 0x00, 0x64, 0x01, 0x56, 0xa8,
	0xc0, 0x00, 0x02, 0x02, 0xc0, 0x00, 0x02, 0x01,
	...
	/* Payload */
	0x80, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,
...
static int verify_echo_reply(struct net_pkt *pkt)
{
	...
	payload_len = sizeof(icmpv4_echo_req) - NET_IPV4H_LEN - NET_ICMPH_LEN;
...
static int verify_echo_reply_with_opts(struct net_pkt *pkt)
{
	...
	/* Calculate payload length */
	payload_len = sizeof(icmpv4_echo_req_opt) - NET_IPV4H_LEN -
		      NET_ICMPH_LEN - opts_len;
	if (payload_len != net_pkt_remaining_data(pkt)) {
	...
	/* Compare the payload */
	if (memcmp(buf,
		   icmpv4_echo_req_opt + NET_IPV4H_LEN + NET_ICMPH_LEN +
			   opts_len,
		   payload_len)) {
...
NET_DEVICE_INIT(net_icmpv4_test, "net_icmpv4_test", net_icmpv4_dev_init,
		device_pm_control_nop, &net_icmpv4_context_data, NULL,
		CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &net_icmpv4_if_api,
...
static struct net_pkt *prepare_echo_request(struct net_if *iface)
{
	...
	pkt = net_pkt_alloc_with_buffer(iface, sizeof(icmpv4_echo_req), AF_INET,
					IPPROTO_ICMP, K_FOREVER);
	if (!pkt) {
		return NULL;
	}
...
static struct net_pkt *prepare_echo_reply(struct net_if *iface)
{
	...
	pkt = net_pkt_alloc_with_buffer(iface, sizeof(icmpv4_echo_rep), AF_INET,
					IPPROTO_ICMP, K_FOREVER);
	if (!pkt) {
		return NULL;
	}
...
prepare_echo_request_with_bad_options(struct net_if *iface)
{
	struct net_pkt *pkt;
...
void test_main(void)
{
	ztest_test_suite(test_icmpv4_fn, ztest_unit_test(test_icmpv4),
