...
static uint8_t query_mdns[] = {
	0xda, 0x0f, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x06, 0x7a, 0x65, 0x70, 0x68, 0x79, 0x72, 0x05,
	0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x00, 0x00, 0x01, 0x00, 0x01,
};
...
static int eval_response1(struct dns_response_test *resp, bool unpack_answer)
{
	...
	if (unpack_answer) {
		...
		if (dns_answer_type(NAME_PTR_SIZE, resp->res) !=
		    resp->answer_type) {
			rc = __LINE__;
			goto lb_exit;
		}
		...
		if (dns_answer_class(NAME_PTR_SIZE, resp->res) !=
		    DNS_CLASS_IN) {
			rc = __LINE__;
			goto lb_exit;
		}
		...
		if (dns_answer_rdlength(NAME_PTR_SIZE, resp->res) !=
		    resp->rdlen) {
			rc = __LINE__;
			goto lb_exit;
		}
...
void test_dns_query(void)
{
	...
	rc = eval_query(DNAME1, tid1, DNS_RR_TYPE_A, query_ipv4,
			sizeof(query_ipv4));
	zassert_equal(rc, 0, "Query test failed for domain: " DNAME1);

	rc = eval_query(NULL, tid1, DNS_RR_TYPE_A, query_ipv4,
			sizeof(query_ipv4));
	zassert_not_equal(rc, 0, "Query test with invalid domain name failed");

	rc = eval_query(DNAME1, tid1, DNS_RR_TYPE_AAAA, query_ipv4,
			sizeof(query_ipv4));
	zassert_not_equal(rc, 0,
			  "Query test for IPv4 with RR type AAAA failed");

	rc = eval_query(DNAME1, tid1 + 1, DNS_RR_TYPE_A, query_ipv4,
			sizeof(query_ipv4));
...
void test_dns_response(void)
{
	...
	zassert_equal(rc, 0,
		      "Response test failed for domain: " DNAME1 " at line %d",
		      -rc);
 ...
 */
char answer_ipv4[] = { 0x21, 0x21, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00,
		       0x00, 0x00, 0x00, 0x03, 0x77, 0x77, 0x77, 0x09, 0x77,
		       0x69, 0x72, 0x65, 0x73, 0x68, 0x61, 0x72, 0x6b, 0x03,
		       0x6f, 0x72, 0x67, 0x00, 0x00, 0x01, 0x00, 0x01, 0x01,
		       0x61, 0xc0, 0x0c, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00,
		       0x02, 0x58, 0x00, 0x04, 0xae, 0x89, 0x2a, 0x41 };
...
void test_mdns_query(void)
{
	...
	rc = eval_query(ZEPHYR_LOCAL, tid1, DNS_RR_TYPE_A, query_mdns,
			sizeof(query_mdns));
...
static uint8_t resp_ipv6[] = {
	0xf2, 0xb6, 0x80, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
	0x06, 0x7a, 0x65, 0x70, 0x68, 0x79, 0x72, 0x05, 0x6c, 0x6f, 0x63, 0x61,
	0x6c, 0x00, 0x00, 0x1c, 0x00, 0x01, 0x06, 0x7a, 0x65, 0x70, 0x68, 0x79,
	0x72, 0x05, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x00, 0x00, 0x1c, 0x00, 0x01,
	0x00, 0x00, 0x00, 0x1e, 0x00, 0x10, 0xfe, 0x80, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x02, 0x00, 0x5e, 0xff, 0xfe, 0x00, 0x53, 0x37,
};

static const uint8_t resp_ipv6_addr[] = { 0xfe, 0x80, 0x00, 0x00, 0x00, 0x00,
					  0x00, 0x00, 0x02, 0x00, 0x5e, 0xff,
					  0xfe, 0x00, 0x53, 0x37 };
...
static uint8_t resp_truncated_response_ipv4_1[] = {
	...
	0x03, 0x77, 0x77, 0x77, 0x0d, 0x7a, 0x65, 0x70, 0x68, 0x79, 0x72, 0x70,
	0x72, 0x6f, 0x6a, 0x65, 0x63, 0x74, 0x03, 0x6f, 0x72, 0x67, 0x00,
...
static uint8_t resp_valid_response_ipv4_11[] = {
	...
	/* Query string */
	0x0e, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x2d, 0x70, 0x72, 0x6f,
	0x64, 0x2d, 0x32, 0x0d, 0x6e, 0x6f, 0x74, 0x69, 0x66, 0x69, 0x63, 0x61,
	0x74, 0x69, 0x6f, 0x6e, 0x73, 0x05, 0x74, 0x65, 0x61, 0x6d, 0x73, 0x09,
	0x6d, 0x69, 0x63, 0x72, 0x6f, 0x73, 0x6f, 0x66, 0x74, 0x03, 0x63, 0x6f,
	0x6d, 0x00,
	...
	/* Data */
	0x11, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x63, 0x6e, 0x73, 0x2d,
	0x70, 0x72, 0x6f, 0x64, 0x2d, 0x32, 0x0e, 0x74, 0x72, 0x61, 0x66, 0x66,
	0x69, 0x63, 0x6d, 0x61, 0x6e, 0x61, 0x67, 0x65, 0x72, 0x03, 0x6e, 0x65,
	0x74, 0x00,
	...
	/* Data */
	0x14, 0x77, 0x65, 0x73, 0x74, 0x75, 0x73, 0x32, 0x63, 0x6e, 0x73, 0x2d,
	0x70, 0x72, 0x6f, 0x64, 0x2d, 0x32, 0x2d, 0x31, 0x36, 0x07, 0x77, 0x65,
	0x73, 0x74, 0x75, 0x73, 0x32, 0x08, 0x63, 0x6c, 0x6f, 0x75, 0x64, 0x61,
	0x70, 0x70, 0x05, 0x61, 0x7a, 0x75, 0x72, 0x65, 0xc0, 0x39,
...
static void setup_dns_context(struct dns_resolve_context *ctx, int idx,
			      uint16_t dns_id, const uint8_t *query,
			      size_t query_len, enum dns_query_type query_type)
{
...
static void run_dns_malformed_response(const char *test_case, uint8_t *buf,
				       size_t len)
{
	...
	static const uint8_t query[] = { /* Labels */
					 0x03, 0x77, 0x77, 0x77, 0x0d, 0x7a,
					 0x65, 0x70, 0x68, 0x79, 0x72, 0x70,
					 0x72, 0x6f, 0x6a, 0x65, 0x63, 0x74,
					 0x03, 0x6f, 0x72, 0x67, 0x00,
					 /* Query type */
					 0x00, 0x01
	};
	...
	ret = dns_validate_msg(&dns_ctx, &dns_msg, &dns_id, &query_idx, NULL,
			       &query_hash);
	zassert_not_equal(ret, DNS_EAI_ALLDONE,
			  "[%s] DNS message was valid (%d)", test_case, ret);
}

static void run_dns_valid_response(const char *test_case, uint8_t *buf,
				   size_t len)
{
	struct dns_msg_t dns_msg = { 0 };
	uint16_t dns_id = 0;
...
static void run_dns_valid_cname_response(const char *test_case, uint8_t *buf,
...
#define RUN_VALID_CNAME_TEST(test_name, expected_ret)                          \
	run_dns_valid_cname_response(#test_name, test_name, sizeof(test_name), \
				     expected_ret)
...
static void test_dns_id_len(void)
{
	...
	ret = dns_validate_msg(&dns_ctx, &dns_msg, &dns_id, &query_idx, NULL,
			       &query_hash);
	zassert_equal(ret, DNS_EAI_FAIL, "DNS message length check failed (%d)",
		      ret);
}
