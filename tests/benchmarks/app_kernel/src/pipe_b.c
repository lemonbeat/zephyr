...
#define PRINT_1_TO_N()                                                      \
	PRINT_F(output_file,                                                \
		...
		putsize, getsize, puttime[0] / 1000.0, puttime[1] / 1000.0, \
		puttime[2] / 1000.0,                                        \
		(1000.0 * putsize) / SAFE_DIVISOR(puttime[0]),              \
		(1000.0 * putsize) / SAFE_DIVISOR(puttime[1]),              \
		(1000.0 * putsize) / SAFE_DIVISOR(puttime[2]))
...
#define PRINT_ALL_TO_N()                                                  \
	PRINT_F(output_file, "|%5u|%5u|%10u|%10u|%10u|%10u|%10u|%10u|\n", \
		putsize, putsize, puttime[0], puttime[1], puttime[2],     \
...
#define PRINT_1_TO_N()                                                    \
	PRINT_F(output_file, "|%5u|%5d|%10u|%10u|%10u|%10u|%10u|%10u|\n", \
		putsize, getsize, puttime[0], puttime[1], puttime[2],     \
 ...
 * Function prototypes.
 */
int pipeput(struct k_pipe *pipe, enum pipe_options option, int size, int count,
	    uint32_t *time);
...
void pipe_test(void)
{
	...
	for (prio = 0; prio < 2; prio++) {
		...
		for (putsize = 8U; putsize <= (MESSAGE_SIZE_PIPE);
		     putsize <<= 1) {
	    ...
	    uint32_t *time)
{
	...
	for (i = 0; option == _1_TO_N || (i < count); i++) {
		...
		ret = k_pipe_put(pipe, data_bench, size2xfer, &sizexferd,
				 mim_num_of_bytes, K_FOREVER);
	...
	*time = SYS_CLOCK_HW_CYCLES_TO_NS_AVG(t, count);
	if (bench_test_end() < 0) {
		if (high_timer_overflow()) {
			...
			PRINT_STRING(
				"| Tick occurred. Results may be inaccurate       ",
				output_file);
		}
		PRINT_STRING("                             |\n", output_file);
	}
