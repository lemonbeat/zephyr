...
void test_qspi_flash(void)
{
	...
	for (i = 0U; i < NUM_OF_SECTORS_TO_TEST; i++) {
		...
		zassert_equal(flash_erase(flash_dev, offset, FLASH_SECTOR_SIZE),
			      TC_PASS, "Flash erase call failed!");
		zassert_equal(flash_read(flash_dev, offset, &rd_val,
					 TEST_DATA_LEN),
		...
		wr_val = 0xAABBCCDD;
		zassert_equal(flash_write(flash_dev, offset, &wr_val,
					  TEST_DATA_LEN),
