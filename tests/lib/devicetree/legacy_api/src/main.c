...
static void test_gpio(void)
{
	...
	zassert_equal(DT_INST_0_VND_GPIO_LABEL, DT_LABEL(DT_INST(0, vnd_gpio)),
		      "");

	zassert_equal(DT_ALIAS_MGR_GPIO_LABEL, DT_LABEL(DT_ALIAS(mgr_gpio)),
		      "");
	...
	zassert_equal(DT_ALIAS_MGR_GPIO_SIZE, DT_REG_SIZE(DT_ALIAS(mgr_gpio)),
		      "");
	...
	zassert_equal(DT_INST_0_VND_GPIO_IRQ_0, DT_IRQN(DT_INST(0, vnd_gpio)),
		      "");

	zassert_equal(DT_ALIAS_MGR_GPIO_IRQ_0, DT_IRQN(DT_ALIAS(mgr_gpio)), "");
...
void test_main(void)
{
	ztest_test_suite(devicetree_legacy_api, ztest_unit_test(test_gpio),
