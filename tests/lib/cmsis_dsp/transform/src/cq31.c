...
static void test_arm_cfft_q31(const arm_cfft_instance_q31 *inst,
			      const q31_t *input, const q31_t *ref,
			      size_t length)
{
	q31_t *output;
...
static void test_arm_cifft_q31(const arm_cfft_instance_q31 *inst,
			       int scale_factor, const q31_t *input,
			       const q31_t *ref, size_t length)
{
	...
	/* Validate output */
	zassert_true(test_snr_error_q31(length, output, scaled_ref,
					SNR_ERROR_THRESH),
		     ASSERT_MSG_SNR_LIMIT_EXCEED);
