...
static void test_arm_correlate_f32(size_t in1_length, size_t in2_length,
				   const uint32_t *ref, size_t ref_length)
{
	...
	/* Run test function */
	arm_correlate_f32((float32_t *)in_com1, in1_length,
			  (float32_t *)in_com2, in2_length, output);

	/* Validate output */
	zassert_true(test_snr_error_f32(ref_length, (float32_t *)ref, output,
					SNR_ERROR_THRESH),
		     ASSERT_MSG_SNR_LIMIT_EXCEED);

	zassert_true(test_close_error_f32(ref_length, (float32_t *)ref, output,
...
#define DEFINE_CORRELATE_TEST(a, b)                            \
	DEFINE_TEST_VARIANT4(arm_correlate_f32, a##_##b, a, b, \
...
static void test_arm_conv_f32(size_t in1_length, size_t in2_length,
			      const uint32_t *ref, size_t ref_length)
{
	float32_t *output;
	...
	/* Run test function */
	arm_conv_f32((float32_t *)in_com1, in1_length, (float32_t *)in_com2,
		     in2_length, output);
...
#define DEFINE_CONV_TEST(a, b)                                                \
	DEFINE_TEST_VARIANT4(arm_conv_f32, a##_##b, a, b, ref_conv_##a##_##b, \
