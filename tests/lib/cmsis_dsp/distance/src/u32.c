			      ...
			      const uint32_t *ref)
{
	...
	for (index = 0; index < length; index++) {
		...
		case OP_ROGERSTANIMOTO:
			val = arm_rogerstanimoto_distance(input1, input2,
							  DIMS_VEC);
			break;
		case OP_RUSSELLRAO:
		...
		case OP_SOKALMICHENER:
			val = arm_sokalmichener_distance(input1, input2,
							 DIMS_VEC);
			break;
		case OP_SOKALSNEATH:
			val = arm_sokalsneath_distance(input1, input2,
						       DIMS_VEC);
			break;
		case OP_YULE:
...
DEFINE_TEST_VARIANT5(arm_distance, dice, OP_DICE, in_dims, in_com1, in_com2,
		     ref_dice);

DEFINE_TEST_VARIANT5(arm_distance, hamming, OP_HAMMING, in_dims, in_com1,
		     in_com2, ref_hamming);

DEFINE_TEST_VARIANT5(arm_distance, jaccard, OP_JACCARD, in_dims, in_com1,
		     in_com2, ref_jaccard);

DEFINE_TEST_VARIANT5(arm_distance, kulsinski, OP_KULSINSKI, in_dims, in_com1,
		     in_com2, ref_kulsinski);

DEFINE_TEST_VARIANT5(arm_distance, rogerstanimoto, OP_ROGERSTANIMOTO, in_dims,
		     in_com1, in_com2, ref_rogerstanimoto);

DEFINE_TEST_VARIANT5(arm_distance, russellrao, OP_RUSSELLRAO, in_dims, in_com1,
		     in_com2, ref_russellrao);

DEFINE_TEST_VARIANT5(arm_distance, sokalmichener, OP_SOKALMICHENER, in_dims,
		     in_com1, in_com2, ref_sokalmichener);

DEFINE_TEST_VARIANT5(arm_distance, sokalsneath, OP_SOKALSNEATH, in_dims,
		     in_com1, in_com2, ref_sokalsneath);

DEFINE_TEST_VARIANT5(arm_distance, yule, OP_YULE, in_dims, in_com1, in_com2,
		     ref_yule);

void test_distance_u32(void)
{
	ztest_test_suite(distance_u32, ztest_unit_test(test_arm_distance_dice),
