...
static void test_arm_distance(int op, bool scratchy, const uint16_t *dims,
			      const uint32_t *dinput1, const uint32_t *dinput2,
			      const uint32_t *ref)
{
	size_t index;
	...
	for (index = 0; index < length; index++) {
		...
		switch (op) {
		case OP_BRAYCURTIS:
			val = arm_braycurtis_distance_f32(input1, input2,
							  DIMS_VEC);
			break;
		case OP_CANBERRA:
			val = arm_canberra_distance_f32(input1, input2,
							DIMS_VEC);
			break;
		case OP_CHEBYSHEV:
			val = arm_chebyshev_distance_f32(input1, input2,
							 DIMS_VEC);
			break;
		case OP_CITYBLOCK:
			val = arm_cityblock_distance_f32(input1, input2,
							 DIMS_VEC);
			break;
		case OP_CORRELATION:
			val = arm_correlation_distance_f32(tmp1, tmp2,
							   DIMS_VEC);
			break;
		case OP_COSINE:
			...
			break;
		case OP_EUCLIDEAN:
			val = arm_euclidean_distance_f32(input1, input2,
							 DIMS_VEC);
			break;
		case OP_JENSENSHANNON:
			val = arm_jensenshannon_distance_f32(input1, input2,
							     DIMS_VEC);
			break;
		default:
	...
	/* Validate output */
	zassert_true(test_near_equal_f32(length, output, (float32_t *)ref,
					 ABS_ERROR_THRESH),
...
DEFINE_TEST_VARIANT6(arm_distance, braycurtis, OP_BRAYCURTIS, false, in_dims,
		     in_com1, in_com2, ref_braycurtis);

DEFINE_TEST_VARIANT6(arm_distance, canberra, OP_CANBERRA, false, in_dims,
		     in_com1, in_com2, ref_canberra);

DEFINE_TEST_VARIANT6(arm_distance, chebyshev, OP_CHEBYSHEV, false, in_dims,
		     in_com1, in_com2, ref_chebyshev);

DEFINE_TEST_VARIANT6(arm_distance, cityblock, OP_CITYBLOCK, false, in_dims,
		     in_com1, in_com2, ref_cityblock);

DEFINE_TEST_VARIANT6(arm_distance, correlation, OP_CORRELATION, true, in_dims,
		     in_com1, in_com2, ref_correlation);

DEFINE_TEST_VARIANT6(arm_distance, cosine, OP_COSINE, false, in_dims, in_com1,
		     in_com2, ref_cosine);

DEFINE_TEST_VARIANT6(arm_distance, euclidean, OP_EUCLIDEAN, false, in_dims,
		     in_com1, in_com2, ref_euclidean);

DEFINE_TEST_VARIANT6(arm_distance, jensenshannon, OP_JENSENSHANNON, false,
		     in_dims, in_jen1, in_jen2, ref_jensenshannon);
...
static void test_arm_distance_minkowski(void)
{
	...
	/* Validate output */
	zassert_true(test_near_equal_f32(length, output,
					 (float32_t *)ref_minkowski,
					 ABS_ERROR_THRESH),
		     ASSERT_MSG_ABS_ERROR_LIMIT_EXCEED);
