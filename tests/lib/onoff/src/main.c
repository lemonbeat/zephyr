...
static void trans_callback(struct onoff_manager *mgr, struct onoff_monitor *mon,
...
static void check_trans(uint32_t idx, uint32_t state, int res, const char *tag)
{
	zassert_true(idx < ntrans, "trans idx %u high: %s", idx, tag);

	const struct transition_record *xp = trans + idx;

	zassert_equal(xp->state, state, "trans[%u] state %x != %x: %s", idx,
		      xp->state, state, tag);
	zassert_equal(xp->res, res, "trans[%u] res %d != %d: %s", idx, xp->res,
		      res, tag);
}
...
static void callback(struct onoff_manager *srv, struct onoff_client *cli,
		     uint32_t state, int res)
{
...
static void check_callback(uint32_t state, int res, const char *tag)
{
	zassert_equal(callback_state, state, "callback state %x != %x: %s",
		      callback_state, state, tag);
	zassert_equal(callback_res, res, "callback res %d != %d: %s",
...
static void check_result(int res, const char *tag)
{
	zassert_equal(cli_result(&cli), res, "cli res %d != %d: %s",
...
static void run_transit(struct onoff_manager *srv, onoff_notify_fn notify,
...
static void start(struct onoff_manager *srv, onoff_notify_fn notify)
{
...
static void stop(struct onoff_manager *srv, onoff_notify_fn notify)
{
...
static void reset(struct onoff_manager *srv, onoff_notify_fn notify)
{
...
static void setup_test(void)
{
	...
	transitions = (struct onoff_transitions)ONOFF_TRANSITIONS_INITIALIZER(
		start, stop, reset);
...
static void setup_error(void)
{
	...
	zassert_equal(rc, ONOFF_STATE_OFF, "req 0 0");
	check_result(start_state.retval, "err req");
	zassert_true(onoff_has_error(&srv), "has_err");
...
static void test_manager_init(void)
{
	...
	zassert_equal(rc, -EINVAL, "init 0 0");
	rc = onoff_manager_init(&srv, NULL);
	zassert_equal(rc, -EINVAL, "init srv 0");
	rc = onoff_manager_init(NULL, &transitions);
	zassert_equal(rc, -EINVAL, "init 0 xit");
	rc = onoff_manager_init(&srv, &xit);
	zassert_equal(rc, -EINVAL, "init 0 xit-start");

	xit.start = start;
	rc = onoff_manager_init(&srv, &xit);
	zassert_equal(rc, -EINVAL, "init srv xit-stop");

	xit.stop = stop;
	rc = onoff_manager_init(&srv, &xit);
	zassert_equal(rc, 0, "init srv xit ok");
}
...
static void test_request(void)
{
	...
	zassert_equal(rc, -EINVAL, "req 0 0");
	rc = onoff_request(&srv, NULL);
	zassert_equal(rc, -EINVAL, "req srv 0");
	rc = onoff_request(NULL, &cli);
	zassert_equal(rc, -EINVAL, "req 0 cli");

	rc = onoff_request(&srv, &cli2);
	zassert_equal(rc, -EINVAL, "req srv cli-uninit");

	cli.notify.flags |= BIT(ONOFF_CLIENT_EXTENSION_POS);
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, -EINVAL, "req srv cli-flags");

	cli.notify.flags &= ~BIT(ONOFF_CLIENT_EXTENSION_POS);
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, 0, "req srv cli ok");

	reset_cli();
	srv.refs = -1;
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, -EAGAIN, "req srv cli ofl");
}
...
static void test_basic_sync(void)
{
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_OFF, "req: %d", rc);
	zassert_equal(srv.refs, 1U, "req refs: %u", srv.refs);
	check_result(start_state.retval, "req");
	zassert_equal(callback_srv, &srv, "callback wrong srv");
	zassert_equal(callback_cli, &cli, "callback wrong cli");
	check_callback(ONOFF_STATE_ON, start_state.retval, "req");
	zassert_equal(ntrans, 2U, "req trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");
	check_trans(1, ONOFF_STATE_ON, start_state.retval, "trans on");

	rc = onoff_release(&srv);
	zassert_equal(rc, ONOFF_STATE_ON, "rel: %d", rc);
	zassert_equal(srv.refs, 0U, "rel refs: %u", srv.refs);
	zassert_equal(ntrans, 4U, "rel trans");
	check_trans(2, ONOFF_STATE_TO_OFF, 0, "trans to-off");
	check_trans(3, ONOFF_STATE_OFF, stop_state.retval, "trans off");

	rc = onoff_release(&srv);
	zassert_equal(rc, -ENOTSUP, "re-rel: %d", rc);
}
...
static void test_basic_async(void)
{
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_OFF, "async req: %d", rc);
	zassert_equal(srv.refs, 0U, "to-on refs: %u", srv.refs);
	check_result(-EAGAIN, "async req");
	zassert_equal(ntrans, 1U, "async req trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");

	notify(&start_state);
	zassert_equal(srv.refs, 1U, "on refs: %u", srv.refs);
	check_result(start_state.retval, "async req");
	zassert_equal(ntrans, 2U, "async req trans");
	check_trans(1, ONOFF_STATE_ON, start_state.retval, "trans on");

	rc = onoff_release(&srv);
	zassert_true(rc >= 0, "rel: %d", rc);
	zassert_equal(srv.refs, 0U, "on refs: %u", srv.refs);
	zassert_equal(ntrans, 3U, "async rel trans");
	check_trans(2, ONOFF_STATE_TO_OFF, 0, "trans to-off");
...
static void test_reset(void)
{
	...
	zassert_equal(rc, -EINVAL, "rst 0 0");
	rc = onoff_reset(&srv, NULL);
	zassert_equal(rc, -EINVAL, "rst srv 0");
	rc = onoff_reset(NULL, &cli);
	zassert_equal(rc, -EINVAL, "rst 0 cli");
	rc = onoff_reset(&srv, &cli2);
	zassert_equal(rc, -EINVAL, "rst srv cli-cfg");

	transitions.reset = NULL;
	rc = onoff_reset(&srv, &cli);
	zassert_equal(rc, -ENOTSUP, "rst srv cli-cfg");

	transitions.reset = reset;
	rc = onoff_reset(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_ERROR, "rst srv cli");

	reset_cli();
	rc = onoff_reset(&srv, &cli);
	zassert_equal(rc, -EALREADY, "re-rst srv cli");
}
...
static void test_basic_reset(void)
{
	...
	zassert_equal(ntrans, 2U, "err trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");
	check_trans(1, ONOFF_STATE_ERROR, start_state.retval, "trans on");

	reset_cli();
	reset_state.retval = 12;

	rc = onoff_reset(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_ERROR, "rst");
	check_result(reset_state.retval, "rst");
	zassert_equal(ntrans, 4U, "err trans");
	check_trans(2, ONOFF_STATE_RESETTING, 0, "trans resetting");
	check_trans(3, ONOFF_STATE_OFF, reset_state.retval, "trans off");
}
...
static void test_multi_start(void)
{
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_OFF, "req: %d", rc);
	zassert_equal(srv.refs, 0U, "req refs: %u", srv.refs);
	check_result(-EAGAIN, "req");
	zassert_equal(ntrans, 1U, "req trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");

	sys_notify_init_spinwait(&cli2.notify);

	rc = onoff_request(&srv, &cli2);
	zassert_equal(rc, ONOFF_STATE_TO_ON, "req2: %d", rc);
	zassert_equal(cli_result(&cli2), -EAGAIN, "req2 result");
	...
	check_trans(1, ONOFF_STATE_ON, start_state.retval, "trans on");
	check_result(start_state.retval, "req");
	zassert_equal(cli_result(&cli2), start_state.retval, "req2");
}
...
static void test_indep_req(void)
{
	...
	zassert_equal(rc, ONOFF_STATE_OFF, "req0: %d", rc);
	zassert_equal(srv.refs, 1U, "req0 refs: %u", srv.refs);
	zassert_equal(cli_result(&cli0), start_state.retval, "req0 result");
	zassert_equal(ntrans, 2U, "req trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");
	check_trans(1, ONOFF_STATE_ON, start_state.retval, "trans on");
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_ON, "req: %d", rc);
	check_result(0, "req");

	zassert_equal(ntrans, 2U, "async req trans");
	zassert_equal(srv.refs, 2U, "srv refs: %u", srv.refs);

	rc = onoff_release(&srv); /* pair with cli0 */
	zassert_equal(rc, ONOFF_STATE_ON, "rel: %d", rc);
	zassert_equal(srv.refs, 1U, "srv refs");
	zassert_equal(ntrans, 2U, "async req trans");

	rc = onoff_release(&srv); /* pair with cli */
	zassert_equal(rc, ONOFF_STATE_ON, "rel: %d", rc);
	zassert_equal(srv.refs, 0U, "srv refs");
	zassert_equal(ntrans, 4U, "async req trans");
}
...
static void test_delayed_req(void)
{
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_TO_OFF, "del req: %d", rc);
	zassert_equal(ntrans, 3U, "async rel trans");
	...
	zassert_equal(ntrans, 6U, "req trans");
	check_trans(2, ONOFF_STATE_TO_OFF, 0, "trans to-off");
	check_trans(3, ONOFF_STATE_OFF, stop_state.retval, "trans off");
	check_trans(4, ONOFF_STATE_TO_ON, 0, "trans to-on");
	check_trans(5, ONOFF_STATE_ON, start_state.retval, "trans on");
}
...
static void test_recheck_start(void)
{
	...
	zassert_equal(rc, 0, "mon unreg");

	start_state.async = true;

	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_OFF, "req");
	rc = onoff_cancel(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_TO_ON, "cancel");

	notify(&start_state);
	zassert_equal(srv.flags, ONOFF_STATE_OFF, "completed");
}
...
static void test_recheck_stop(void)
{
	...
	stop_state.async = true;
	rc = onoff_release(&srv);
	zassert_equal(rc, ONOFF_STATE_ON, "rel");

	reset_cli();
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_TO_OFF, "delayed req");
	check_result(-EAGAIN, "delayed req");

	notify(&stop_state);
	zassert_equal(srv.flags, ONOFF_STATE_ON, "completed");
}

static void rel_in_req_cb(struct onoff_manager *srv, struct onoff_client *cli,
			  uint32_t state, int res)
{
	int rc = onoff_release(srv);

	zassert_equal(rc, ONOFF_STATE_ON, "rel-in-req");
}
...
static void test_rel_in_req_cb(void)
{
	...
	zassert_equal(callback_fn, NULL, "invoke");

	zassert_equal(ntrans, 4U, "req trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");
	check_trans(1, ONOFF_STATE_ON, start_state.retval, "trans on");
	check_trans(2, ONOFF_STATE_TO_OFF, 0, "trans to-off");
	check_trans(3, ONOFF_STATE_OFF, stop_state.retval, "trans off");
...
static void test_multi_reset(void)
{
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_OFF, "req err");
	check_result(start_state.retval, "req err");
	zassert_true(onoff_has_error(&srv), "has_error");
	zassert_equal(ntrans, 2U, "err trans");
	check_trans(0, ONOFF_STATE_TO_ON, 0, "trans to-on");
	check_trans(1, ONOFF_STATE_ERROR, start_state.retval, "trans on");
	...
	sys_notify_init_spinwait(&cli2.notify);
	rc = onoff_reset(&srv, &cli2);
	zassert_equal(rc, ONOFF_STATE_ERROR, "rst2");
	zassert_equal(cli_result(&cli2), -EAGAIN, "rst2 result");
	zassert_equal(ntrans, 3U, "rst trans");
	check_trans(2, ONOFF_STATE_RESETTING, 0, "trans resetting");

	reset_cli();
	rc = onoff_reset(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_RESETTING, "rst");
	zassert_equal(ntrans, 3U, "rst trans");

	notify(&reset_state);

	zassert_equal(cli_result(&cli2), reset_state.retval, "rst2 result");
	check_result(reset_state.retval, "rst");
	zassert_equal(ntrans, 4U, "rst trans");
	check_trans(3, ONOFF_STATE_OFF, reset_state.retval, "trans off");
}
...
static void test_error(void)
{
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, -EIO, "req in err");

	rc = onoff_release(&srv);
	zassert_equal(rc, -EIO, "rel in err");
	...
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, -ENOTSUP, "req in err");

	rc = onoff_release(&srv);
	zassert_equal(rc, -ENOTSUP, "rel in err");
}
...
static void test_cancel_req(void)
{
	...
	zassert_equal(rc, -EINVAL, "can 0 0");
	rc = onoff_cancel(&srv, NULL);
	zassert_equal(rc, -EINVAL, "can srv 0");
	rc = onoff_cancel(NULL, &cli);
	zassert_equal(rc, -EINVAL, "can 0 cli");
	...
	rc = onoff_cancel(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_TO_ON, "cancel req: %d", rc);

	rc = onoff_cancel(&srv, &cli);
	zassert_equal(rc, -EALREADY, "re-cancel req: %d", rc);
...
static void test_cancel_delayed_req(void)
{
	...
	rc = onoff_cancel(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_TO_OFF, "can del req: %d", rc);
...
static void test_cancel_or_release(void)
{
	...
	zassert_equal(rc, ONOFF_STATE_TO_ON, "c|r to-on");
	notify(&start_state);
	...
	rc = onoff_cancel_or_release(&srv, &cli);
	zassert_equal(rc, ONOFF_STATE_ON, "c|r to-on");
	zassert_equal(ntrans, 4U, "req trans");
	check_trans(3, ONOFF_STATE_OFF, stop_state.retval, "trans off");
}
...
static void test_sync_basic(void)
{
	...
	zassert_equal(rc, 0, "init req");

	rc = onoff_sync_finalize(&srv, key, &cli, res, true);
	zassert_equal(rc, 1, "req count");
	zassert_equal(callback_srv, NULL, "sync cb srv");
	zassert_equal(callback_cli, &cli, "sync cb cli");
	...
	rc = onoff_sync_lock(&srv, &key);
	zassert_equal(rc, 1, "init rel");

	++res;
	rc = onoff_sync_finalize(&srv, key, &cli, res, true);
	zassert_equal(rc, 2, "req2 count");
	check_callback(ONOFF_STATE_ON, res, "sync req2");

	reset_cli();

	rc = onoff_sync_lock(&srv, &key);
	zassert_equal(rc, 2, "init rel");

	rc = onoff_sync_finalize(&srv, key, NULL, res, false);
	zassert_equal(rc, 1, "rel count");

	reset_cli();

	rc = onoff_sync_lock(&srv, &key);
	zassert_equal(rc, 1, "init rel2");

	rc = onoff_sync_finalize(&srv, key, NULL, res, false);
	zassert_equal(rc, 0, "rel2 count");
	...
	rc = onoff_sync_lock(&srv, &key);
	zassert_equal(rc, 0, "init rel2");

	rc = onoff_sync_finalize(&srv, key, NULL, res, false);
	zassert_equal(rc, -1, "rel-1 count");
...
static void test_sync_error(void)
{
	...
	zassert_equal(rc, res, "err final");
	zassert_equal(srv.count, res, "srv err count");
	zassert_equal(callback_srv, NULL, "sync cb srv");
	zassert_equal(callback_cli, &cli, "sync cb cli");
	...
	zassert_equal(rc, 1, "req count %d", rc);
	check_callback(ONOFF_STATE_ON, res, "sync req");
...
void test_main(void)
{
	...
	ztest_test_suite(
		onoff_api, ztest_unit_test(test_manager_init),
		ztest_unit_test(test_mon_reg), ztest_unit_test(test_mon_unreg),
		ztest_unit_test(test_request), ztest_unit_test(test_basic_sync),
		ztest_unit_test(test_basic_async), ztest_unit_test(test_reset),
		...
		ztest_unit_test(test_multi_reset), ztest_unit_test(test_error),
