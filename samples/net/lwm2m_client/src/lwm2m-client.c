...
static unsigned char client_psk[] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
static int firmware_block_received_cb(uint16_t obj_inst_id, uint16_t res_id,
				      ...
				      size_t total_size)
{
	LOG_INF("FIRMWARE: BLOCK RECEIVED: len:%u last_block:%d", data_len,
		last_block);
	return 0;
}
#endif

static int timer_digital_state_cb(uint16_t obj_inst_id, uint16_t res_id,
				  uint16_t res_inst_id, uint8_t *data,
				  uint16_t data_len, bool last_block,
				  size_t total_size)
{
...
static int lwm2m_setup(void)
{
	...
	lwm2m_engine_set_opaque("0/0/5", (void *)client_psk,
				sizeof(client_psk));
#endif /* CONFIG_LWM2M_DTLS_SUPPORT */
	...
	if (init_led_device() == 0) {
		...
		lwm2m_engine_register_post_write_callback("3311/0/5850",
							  led_on_off_cb);
		lwm2m_engine_set_res_data("3311/0/5750", LIGHT_NAME,
					  sizeof(LIGHT_NAME),
					  LWM2M_RES_DATA_FLAG_RO);
	}
