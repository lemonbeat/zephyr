...
struct pi_lcd_data lcd_data = { .disp_func = LCD_4BIT_MODE | LCD_1_LINE |
					     LCD_5x8_DOTS,
				.disp_cntl = 0,
				.disp_mode = 0,
				.cfg_rows = 0,
...
void pi_lcd_set_cursor(const struct device *gpio_dev, uint8_t col, uint8_t row)
{
	...
	_pi_lcd_command(gpio_dev, (LCD_SET_DDRAM_ADDR |
				   (col + lcd_data.row_offsets[row])));
}
...
void pi_lcd_display_on(const struct device *gpio_dev)
{
	...
	_pi_lcd_command(gpio_dev, LCD_DISPLAY_CONTROL | lcd_data.disp_cntl);
}
...
void pi_lcd_scroll_left(const struct device *gpio_dev)
{
	_pi_lcd_command(gpio_dev,
			LCD_CURSOR_SHIFT | LCD_DISPLAY_MOVE | LCD_MOVE_LEFT);
}
...
{
	_pi_lcd_command(gpio_dev,
			LCD_CURSOR_SHIFT | LCD_DISPLAY_MOVE | LCD_MOVE_RIGHT);
}
...
void pi_lcd_left_to_right(const struct device *gpio_dev)
{
	...
	_pi_lcd_command(gpio_dev, LCD_ENTRY_MODE_SET | lcd_data.disp_cntl);
}
