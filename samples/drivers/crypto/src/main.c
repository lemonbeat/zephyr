...
void ecb_mode(const struct device *dev)
{
	...
	uint8_t ecb_plaintext[16] = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
				      0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
				      0xCC, 0xDD, 0xEE, 0xFF };
	const uint8_t ecb_ciphertext[16] = { 0x69, 0xC4, 0xE0, 0xD8, 0x6A, 0x7B,
...
void cbc_mode(const struct device *dev)
{
	...
	static uint8_t iv[16] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
...
static const uint8_t ctr_ciphertext[64] = {
	0x22, 0xe5, 0x2f, 0xb1, 0x77, 0xd8, 0x65, 0xb2, 0xf7, 0xc6, 0xb5,
	0x12, 0x69, 0x2d, 0x11, 0x4d, 0xed, 0x6c, 0x1c, 0x72, 0x25, 0xda,
	0xf6, 0xa2, 0xaa, 0xd9, 0xd3, 0xda, 0x2d, 0xba, 0x21, 0x68, 0x35,
	0xc0, 0xaf, 0x6b, 0x6f, 0x40, 0xc3, 0xc6, 0xef, 0xc5, 0x85, 0xd0,
	0x90, 0x2c, 0xc2, 0x63, 0x12, 0x2b, 0xc5, 0x8e, 0x72, 0xde, 0x5c,
	0xa2, 0xa3, 0x5c, 0x85, 0x3a, 0xb9, 0x2c, 0x6,	0xbb
};
...
void ctr_mode(const struct device *dev)
{
	...
	uint8_t iv[12] = { 0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5,
...
void ccm_mode(const struct device *dev)
{
	...
	if (memcmp(encrypt.out_buf, ccm_expected, sizeof(ccm_expected))) {
		LOG_ERR("CCM mode ENCRYPT - Mismatch between expected "
			"and returned cipher text");
		print_buffer_comparison(ccm_expected, encrypt.out_buf,
					sizeof(ccm_expected));
		goto out;
	}
	...
	if (memcmp(decrypt.out_buf, ccm_data, sizeof(ccm_data))) {
		...
		print_buffer_comparison(ccm_data, decrypt.out_buf,
					sizeof(ccm_data));
		goto out;
	}
...
static uint8_t gcm_hdr[20] = { 0xe2, 0x01, 0x06, 0xd7, 0xcd, 0x0d, 0xf0,
			       0x76, 0x1e, 0x8d, 0xcd, 0x3d, 0x88, 0xe5,
			       0x4c, 0x2a, 0x76, 0xd4, 0x57, 0xed };
static uint8_t gcm_data[42] = { 0x08, 0x00, 0x0f, 0x10, 0x11, 0x12, 0x13,
				0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a,
				0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21,
				0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28,
				0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f,
				0x30, 0x31, 0x32, 0x33, 0x34, 0x00, 0x04 };
...
void gcm_mode(const struct device *dev)
{
	...
	if (memcmp(encrypt.out_buf, gcm_expected, sizeof(gcm_expected))) {
		LOG_ERR("GCM mode ENCRYPT - Mismatch between expected "
			"and returned cipher text");
		print_buffer_comparison(gcm_expected, encrypt.out_buf,
					sizeof(gcm_expected));
		goto out;
	}
	...
	if (memcmp(decrypt.out_buf, gcm_data, sizeof(gcm_data))) {
		LOG_ERR("GCM mode DECRYPT - Mismatch between plaintext "
			"and decrypted cipher text");
		print_buffer_comparison(gcm_data, decrypt.out_buf,
					sizeof(gcm_data));
		goto out;
	}
