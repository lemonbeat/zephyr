...
const struct isotp_msg_id rx_addr_8_0 = { .std_id = 0x80,
					  .id_type = CAN_STANDARD_IDENTIFIER,
					  .use_ext_addr = 0 };
const struct isotp_msg_id tx_addr_8_0 = { .std_id = 0x180,
					  .id_type = CAN_STANDARD_IDENTIFIER,
					  .use_ext_addr = 0 };
const struct isotp_msg_id rx_addr_0_5 = { .std_id = 0x01,
					  .id_type = CAN_STANDARD_IDENTIFIER,
					  .use_ext_addr = 0 };
const struct isotp_msg_id tx_addr_0_5 = { .std_id = 0x101,
					  .id_type = CAN_STANDARD_IDENTIFIER,
					  .use_ext_addr = 0 };
...
void rx_8_0_thread(void *arg1, void *arg2, void *arg3)
{
	...
	ret = isotp_bind(&recv_ctx_8_0, can_dev, &tx_addr_8_0, &rx_addr_8_0,
...
void rx_0_5_thread(void *arg1, void *arg2, void *arg3)
{
	...
	ret = isotp_bind(&recv_ctx_0_5, can_dev, &tx_addr_0_5, &rx_addr_0_5,
...
void main(void)
{
	...
	while (1) {
		k_msleep(1000);
		ret = isotp_send(&send_ctx_0_5, can_dev, tx_data_small,
				 sizeof(tx_data_small), &tx_addr_0_5,
				 &rx_addr_0_5, send_complette_cb, NULL);
		if (ret != ISOTP_N_OK) {
			printk("Error while sending data to ID %d [%d]\n",
			       tx_addr_0_5.std_id, ret);
		}

		ret = isotp_send(&send_ctx_8_0, can_dev, tx_data_large,
				 sizeof(tx_data_large), &tx_addr_8_0,
				 &rx_addr_8_0, NULL, NULL);
		if (ret != ISOTP_N_OK) {
			printk("Error while sending data to ID %d [%d]\n",
