...
static void fault_show(const z_arch_esf_t *esf, int fault)
{
	...
	PR_EXC("MMFSR: 0x%x, BFSR: 0x%x, UFSR: 0x%x", SCB_MMFSR, SCB_BFSR,
	       SCB_UFSR);
				 ...
				 bool *recoverable)
{
	...
	if (SCB->CFSR & SCB_CFSR_MSTKERR_Msk) {
		...
		if (SCB->ICSR & SCB_ICSR_RETTOBASE_Msk) {
			uint32_t min_stack_ptr = z_check_thread_stack_fail(
				mmfar, ((uint32_t)&esf[0]));
...
#else
				(void)ear;
				__ASSERT(0, "Stacking error without stack guard"
...
static void debug_monitor(const z_arch_esf_t *esf)
{
	...
	PR_FAULT_INFO("***** Debug monitor exception (not implemented) *****");
}
 ...
 */
static inline z_arch_esf_t *get_esf(uint32_t msp, uint32_t psp,
