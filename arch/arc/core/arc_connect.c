...
void z_arc_connect_ici_clear(void)
{
	 ...
	 */
		while (cpu) {
			...
			z_arc_connect_cmd(ARC_CONNECT_CMD_INTRPT_GENERATE_ACK,
					  c);
			cpu &= ~(1U << c);
		}
	}
...
void z_arc_connect_debug_reset(uint32_t core_mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_DEBUG_RESET, 0,
				       core_mask);
	}
}
...
void z_arc_connect_debug_halt(uint32_t core_mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_DEBUG_HALT, 0,
				       core_mask);
	}
}
...
void z_arc_connect_debug_run(uint32_t core_mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_DEBUG_RUN, 0, core_mask);
	}
}
...
void z_arc_connect_debug_mask_set(uint32_t core_mask, uint32_t mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_DEBUG_SET_MASK, mask,
				       core_mask);
	}
}
...
uint32_t z_arc_connect_debug_mask_read(uint32_t core_mask)
{
	...
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_DEBUG_READ_MASK, 0,
				       core_mask);
		ret = z_arc_connect_cmd_readback();
	}
...
void z_arc_connect_debug_select_set(uint32_t core_mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_DEBUG_SET_SELECT, 0,
				       core_mask);
	}
}
...
void z_arc_connect_gfrc_core_set(uint32_t core_mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_GFRC_SET_CORE, 0,
				       core_mask);
	}
}
 ...
 * interrupt
 */
void z_arc_connect_idu_set_mode(uint32_t irq_num, uint16_t trigger_mode,
				uint16_t distri_mode)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_IDU_SET_MODE, irq_num,
				       (distri_mode | (trigger_mode << 4)));
	}
}
...
void z_arc_connect_idu_set_dest(uint32_t irq_num, uint32_t core_mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_IDU_SET_DEST, irq_num,
				       core_mask);
	}
}
...
void z_arc_connect_idu_set_mask(uint32_t irq_num, uint32_t mask)
{
	LOCKED(&arc_connect_spinlock)
	{
		z_arc_connect_cmd_data(ARC_CONNECT_CMD_IDU_SET_MASK, irq_num,
				       mask);
	}
}
