		     ...
		     void *p2, void *p3)
{
	thread->switch_handle =
		xtensa_init_stack((int *)stack_ptr, entry, p1, p2, p3);
}
...
void z_irq_spurious(const void *arg)
{
	...
	LOG_ERR(" ** Spurious INTERRUPT(s) %p, INTENABLE = %p", (void *)irqs,
		(void *)ie);
...
void z_xtensa_dump_stack(const z_arch_esf_t *stack)
{
	...
	LOG_ERR(" **  A0 %p  SP %p  A2 %p  A3 %p", (void *)bsa[BSA_A0_OFF / 4],
		((char *)bsa) + BASE_SAVE_AREA_SIZE,
		(void *)bsa[BSA_A2_OFF / 4], (void *)bsa[BSA_A3_OFF / 4]);

	if (bsa - stack > 4) {
		LOG_ERR(" **  A4 %p  A5 %p  A6 %p  A7 %p", (void *)bsa[-4],
			(void *)bsa[-3], (void *)bsa[-2], (void *)bsa[-1]);
	}

	if (bsa - stack > 8) {
		LOG_ERR(" **  A8 %p  A9 %p A10 %p A11 %p", (void *)bsa[-8],
			(void *)bsa[-7], (void *)bsa[-6], (void *)bsa[-5]);
	}

	if (bsa - stack > 12) {
		LOG_ERR(" ** A12 %p A13 %p A14 %p A15 %p", (void *)bsa[-12],
			(void *)bsa[-11], (void *)bsa[-10], (void *)bsa[-9]);
	}

#if XCHAL_HAVE_LOOPS
	LOG_ERR(" ** LBEG %p LEND %p LCOUNT %p", (void *)bsa[BSA_LBEG_OFF / 4],
		(void *)bsa[BSA_LEND_OFF / 4], (void *)bsa[BSA_LCOUNT_OFF / 4]);
#endif
	...
	void *xtensa_excint1_c(int *interrupted_stack)
{
	...
	if (cause == EXCCAUSE_LEVEL1_INTERRUPT) {
		...
		LOG_ERR(" ** SYSCALL PS %p PC %p", (void *)bsa[BSA_PS_OFF / 4],
			(void *)bsa[BSA_PC_OFF / 4]);
		...
		LOG_ERR(" ** CPU %d EXCCAUSE %d (%s)", arch_curr_cpu()->id,
			cause, z_xtensa_exccause(cause));
		LOG_ERR(" **  PC %p VADDR %p", (void *)bsa[BSA_PC_OFF / 4],
			(void *)vaddr);
		...
		LOG_ERR(" **    (INTLEVEL:%d EXCM: %d UM:%d RING:%d WOE:%d OWB:%d CALLINC:%d)",
			...
			get_bits(18, 1, ps), get_bits(8, 4, ps),
			get_bits(16, 2, ps));
