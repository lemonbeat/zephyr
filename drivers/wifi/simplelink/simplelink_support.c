...
static int32_t configure_simplelink(void)
{
	...
	retval = sl_WlanPolicySet(SL_WLAN_POLICY_CONNECTION,
				  SL_WLAN_CONNECTION_POLICY(1, 1, 0, 0), NULL,
				  0);
	...
	ipV4.Ip = (_u32)SL_IPV4_VAL(addr4.s4_addr[0], addr4.s4_addr[1],
	...
	if (strcmp(CONFIG_NET_CONFIG_MY_IPV4_GW, "") != 0) {
		...
		ipV4.IpGateway =
			(_u32)SL_IPV4_VAL(addr4.s4_addr[0], addr4.s4_addr[1],
					  addr4.s4_addr[2], addr4.s4_addr[3]);
	}
#endif
...
#else
	/* enable DHCP client */
	retval = sl_NetCfgSet(SL_NETCFG_IPV4_STA_ADDR_MODE, SL_NETCFG_ADDR_DHCP,
			      0, 0);
	ASSERT_ON_ERROR(retval, NETAPP_ERROR);
#endif
...
void SimpleLinkWlanEventHandler(SlWlanEvent_t *wlan_event)
{
	...
	case SL_WLAN_EVENT_CONNECT:
		...
		LOG_INF("[WLAN EVENT] STA Connected to the AP: %s, "
			...
			sl_conn.bssid[1], sl_conn.bssid[2], sl_conn.bssid[3],
	...
	case SL_WLAN_EVENT_DISCONNECT:
		...
		if (SL_WLAN_DISCONNECT_USER_INITIATED ==
			...
			LOG_INF("BSSID: %x:%x:%x:%x:%x:%x on application's"
				" request",
				event_data->Bssid[0], event_data->Bssid[1],
				event_data->Bssid[2], event_data->Bssid[3],
	...
	case SL_WLAN_EVENT_STA_ADDED:
		...
		LOG_INF("[WLAN EVENT] STA was added to AP: "
			"BSSID: %x:%x:%x:%x:%x:%x",
			sl_conn.bssid[0], sl_conn.bssid[1], sl_conn.bssid[2],
...
void SimpleLinkNetAppEventHandler(SlNetAppEvent_t *netapp_event)
{
	...
	if ((netapp_event->Id == SL_NETAPP_EVENT_IPV4_ACQUIRED) ||
		 ...
		 */
		SlNetIf_add(
			SLNETIF_ID_1, SLNET_IF_WIFI_NAME,
...
void SimpleLinkGeneralEventHandler(SlDeviceEvent_t *dev_event)
{
	...
	LOG_INF("[GENERAL EVENT] - ID=[%d] Sender=[%d]",
		dev_event->Data.Error.Code, dev_event->Data.Error.Source);
}
...
void z_simplelink_get_mac(unsigned char *mac)
{
	...
	sl_NetCfgGet(SL_NETCFG_MAC_ADDRESS_GET, &config_opt, &mac_len,
		     (uint8_t *)mac);
}
