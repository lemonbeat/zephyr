...
DEVICE_AND_API_INIT(ipm_nrf, DT_INST_LABEL(0), ipm_nrf_init, NULL, NULL,
...
#define VIPM_DEVICE_1(_idx)                                                   \
	...
	{                                                                     \
		...
		return 0;                                                     \
                                                                              ...
                                                                              \
	static void vipm_nrf_##_idx##_register_callback(                      \
		const struct device *dev, ipm_callback_t cb, void *user_data) \
	{                                                                     \
		if (IS_ENABLED(CONFIG_IPM_MSG_CH_##_idx##_RX)) {              \
			...
			LOG_WRN("Trying to register a callback"               \
				"for TX channel IPM_" #_idx);                 \
                                                                              ...
                                                                              \
	static int vipm_nrf_##_idx##_set_enabled(const struct device *dev,    \
						 int enable)                  \
	{                                                                     \
		if (!IS_ENABLED(CONFIG_IPM_MSG_CH_##_idx##_RX)) {             \
	...
	static const struct ipm_driver_api vipm_nrf_##_idx##_driver_api = {   \
		...
		.set_enabled = vipm_nrf_##_idx##_set_enabled                  \
                                                                              ...
                                                                              \
	DEVICE_AND_API_INIT(vipm_nrf_##_idx, "IPM_" #_idx, vipm_nrf_init,     \
