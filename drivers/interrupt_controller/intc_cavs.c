...
static ALWAYS_INLINE struct cavs_registers *
get_base_address(struct cavs_ictl_runtime *context)
{
#if defined(CONFIG_SMP) && (CONFIG_MP_NUM_CPUS > 1)
...
cavs_ictl_dispatch_child_isrs(uint32_t intr_status, uint32_t isr_base_offset)
{
...
#define CAVS_ICTL_INIT(n)                                                    \
	...
	DEVICE_AND_API_INIT(cavs_ictl_##n, DT_INST_LABEL(n),                 \
			    cavs_ictl_##n##_initialize, &cavs_##n##_runtime, \
			    &cavs_config_##n, PRE_KERNEL_1,                  \
