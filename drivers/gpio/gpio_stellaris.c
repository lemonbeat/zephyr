...
#define DEV_CFG(dev) ((const struct gpio_stellaris_config *const)(dev)->config)

#define DEV_DATA(dev) ((struct gpio_stellaris_runtime *const)(dev)->data)
...
static int gpio_stellaris_configure(const struct device *dev, gpio_pin_t pin,
				    gpio_flags_t flags)
{
						  ...
						  enum gpio_int_trig trig)
{
	...
	if (mode == GPIO_INT_MODE_DISABLED) {
		...
		if (trig == GPIO_INT_TRIG_BOTH) {
			...
			sys_clear_bit(GPIO_REG_ADDR(base, GPIO_IEV_OFFSET),
				      pin);
		}
...
#define STELLARIS_GPIO_DEVICE(n)                                                          \
	static void port_##n##_stellaris_config_func(                                     \
	...
	static const struct gpio_stellaris_config gpio_stellaris_port_## n ##_config = {\
		...
		.config_func = port_## n ##_stellaris_config_func,			\
                                                                                          ...
                                                                                          \
	DEVICE_AND_API_INIT(gpio_stellaris_port_##n, DT_INST_LABEL(n),                    \
			    gpio_stellaris_init,                                          \
