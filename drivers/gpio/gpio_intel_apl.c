...
#define DEV_CFG(_dev) ((const struct gpio_intel_apl_config *)(_dev)->config)
...
static int gpio_intel_apl_config(const struct device *dev, gpio_pin_t pin,
				 gpio_flags_t flags)
{
...
int gpio_intel_apl_init(const struct device *dev)
{
	...
	if (nr_isr_devs == 0) {
		...
		IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
			    gpio_intel_apl_isr, NULL, DT_INST_IRQ(0, sense));
...
#define GPIO_INTEL_APL_DEV_CFG_DATA(n)                                     \
	...
	static struct gpio_intel_apl_data gpio_intel_apl_data_##n;         \
                                                                           \
	DEVICE_AND_API_INIT(gpio_intel_apl_##n, DT_INST_LABEL(n),          \
			    gpio_intel_apl_init, &gpio_intel_apl_data_##n, \
			    &gpio_intel_apl_cfg_##n, POST_KERNEL,          \
