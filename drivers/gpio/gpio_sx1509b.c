...
static int sx1509b_handle_interrupt(const struct device *dev)
{
	...
	ret = i2c_write_read(drv_data->i2c_master, cfg->i2c_slave_addr, &cmd,
			     sizeof(cmd), (uint8_t *)&int_source,
			     sizeof(int_source));
	if (ret != 0) {
		goto out;
	}
...
{
	struct sx1509b_drv_data *drv_data =
		CONTAINER_OF(gpio_cb, struct sx1509b_drv_data, gpio_cb);
...
static int sx1509b_config(const struct device *dev, gpio_pin_t pin,
			  gpio_flags_t flags)
{
	const struct sx1509b_config *cfg = dev->config;
	struct sx1509b_drv_data *drv_data = dev->data;
	...
	if (rc == 0) {
		...
		rc = i2c_write(drv_data->i2c_master, &debounce_buf.reg,
			       sizeof(debounce_buf), cfg->i2c_slave_addr);
	}
...
static int port_get(const struct device *dev, gpio_port_value_t *value)
{
	...
	rc = i2c_write_read(drv_data->i2c_master, cfg->i2c_slave_addr, &cmd,
			    sizeof(cmd), &pin_data, sizeof(pin_data));
		      ...
		      gpio_port_value_t value, gpio_port_value_t toggle)
{
	...
	int rc = i2c_reg_write_word_be(drv_data->i2c_master,
	...
	LOG_DBG("write %04x msk %04x val %04x => %04x: %d", orig_out, mask,
		value, out, rc);
				   ...
				   enum gpio_int_trig trig)
{
	...
	if (mode == GPIO_INT_MODE_DISABLED) {
		...
		if (trig == GPIO_INT_TRIG_BOTH) {
			irq->interrupt_sense |=
				(SX1509B_EDGE_BOTH << (pin * 2));
		} else if (trig == GPIO_INT_TRIG_LOW) {
			irq->interrupt_sense |=
				(SX1509B_EDGE_FALLING << (pin * 2));
		} else if (trig == GPIO_INT_TRIG_HIGH) {
			irq->interrupt_sense |=
				(SX1509B_EDGE_RISING << (pin * 2));
		}
	}
...
static int sx1509b_init(const struct device *dev)
{
	...
	if (!drv_data->i2c_master) {
		LOG_ERR("%s: no bus %s", dev->name, cfg->i2c_master_dev_name);
		rc = -EINVAL;
		goto out;
	}
...
DEVICE_AND_API_INIT(sx1509b, DT_INST_LABEL(0), sx1509b_init, &sx1509b_drvdata,
		    &sx1509b_cfg, POST_KERNEL,
