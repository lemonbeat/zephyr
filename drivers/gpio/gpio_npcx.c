...
/* GPIO api functions */
static int gpio_npcx_config(const struct device *dev, gpio_pin_t pin,
			    gpio_flags_t flags)
{
					     ...
					     enum gpio_int_trig trig)
{
	...
	LOG_DBG("pin_int_conf (%d, %d) match (%d, %d, %d)!!!", config->port,
		pin, config->wui_maps[pin].table, config->wui_maps[pin].group,
	...
	soc_miwu_interrupt_configure(&config->wui_maps[pin], miwu_mode,
				     miwu_trig);
...
#define NPCX_GPIO_DEVICE_INIT(inst)                                              \
	...
	static struct gpio_npcx_data gpio_npcx_data_##inst;                      \
                                                                                 \
	DEVICE_AND_API_INIT(gpio_npcx_##inst, DT_INST_LABEL(inst),               \
			    gpio_npcx_init, &gpio_npcx_data_##inst,              \
			    &gpio_npcx_cfg_##inst, POST_KERNEL,                  \
...
static const struct device *gpio_devs[] = { DT_INST_FOREACH_STATUS_OKAY(
	NPCX_GPIO_DEV) };
