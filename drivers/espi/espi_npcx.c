...
#define NPCX_OOB_RX_PACKAGE_LEN(hdr) \
	(((hdr & 0xff000000) >> 24) | ((hdr & 0xf0000) >> 8))
...
#define NPCX_ESPI_BUS_INT_ITEM(event, isr)                             \
	{                                                              \
		...
		.wake_en_bit = NPCX_ESPIWE_##event##WE, .bus_isr = isr \
	}
...
static void espi_bus_cfg_update_isr(const struct device *dev)
{
	...
	chg_mask &=
		(ESPI_CHANNEL_VWIRE | ESPI_CHANNEL_OOB | ESPI_CHANNEL_FLASH);
	...
	LOG_DBG("ESPI CFG EC Updated! 0x%02X",
		GET_FIELD(inst->ESPICFG, NPCX_ESPICFG_CHANS_FIELD));
...
static void espi_vw_send_bootload_done(const struct device *dev)
{
	...
	ret = espi_npcx_receive_vwire(dev, ESPI_VWIRE_SIGNAL_SLV_BOOT_DONE,
				      &boot_done);
...
static void espi_vw_generic_isr(const struct device *dev, struct npcx_wui *wui)
{
	...
	if (signal == ARRAY_SIZE(vw_in_tbl))
		LOG_ERR("Unknown VW event! %d %d %d", wui->table, wui->group,
			wui->bit);
...
static void espi_vw_espi_rst_isr(const struct device *dev, struct npcx_wui *wui)
{
	...
	data->espi_rst_asserted =
		!IS_BIT_SET(inst->ESPISTS, NPCX_ESPISTS_ESPIRST_LVL);
...
static int espi_npcx_configure(const struct device *dev, struct espi_cfg *cfg)
{
	...
	LOG_DBG("%s: %d %d ESPICFG: 0x%08X", __func__, max_freq, io_mode,
		inst->ESPICFG);
			      ...
			      struct espi_oob_packet *pckt)
{
	...
	for (idx_tx_buf = 0; idx_tx_buf < sz_oob_tx / 4;
	     idx_tx_buf++, oob_buf += 4)
		inst->OOBTXBUF[idx_tx_buf + 1] =
			oob_buf[0] | (oob_buf[1] << 8) | (oob_buf[2] << 16) |
			(oob_buf[3] << 24);
...
DEVICE_AND_API_INIT(espi_npcx_0, DT_INST_LABEL(0), &espi_npcx_init,
		    &espi_npcx_data, &espi_npcx_config, PRE_KERNEL_2,
...
static int espi_npcx_init(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
		    espi_bus_generic_isr, DEVICE_GET(espi_npcx_0), 0);
