...
#define GNA_NUM_PAGES(bytes) GNA_SHIFT_RNDUP((bytes), GNA_PG_SIZE_IN_BITSHIFT)
...
#define GNA_GET_BITS(val, b_hi, b_lo) \
	((((uint32_t)(val)) << (31 - (b_hi))) >> (31 - (b_hi) + (b_lo)))

#define GNA_VA_PG_DIR(virt_addr) GNA_GET_BITS(virt_addr, 27, 22)
#define GNA_VA_PG_TABLE(virt_addr) GNA_GET_BITS(virt_addr, 21, 12)

#define GNA_PHYS_ADDR_TO_PAGE(addr) \
	((uint32_t)(addr) >> GNA_PG_SIZE_IN_BITSHIFT)
#define GNA_PG_DIR_ENTRY(phys_addr) GNA_PHYS_ADDR_TO_PAGE(phys_addr)
#define GNA_PG_BASE(addr) \
	((uint32_t)(addr) & ~BIT_MASK(GNA_PG_SIZE_IN_BITSHIFT))
#define GNA_PG_OFFSET(addr) ((uint32_t)(addr)&BIT_MASK(GNA_PG_SIZE_IN_BITSHIFT))
