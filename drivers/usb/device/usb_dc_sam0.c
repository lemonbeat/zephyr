...
#else
	pad_trim =
		(*((uint32_t *)(NVMCTRL_OTP4) + (NVM_USB_PAD_TRIM_POS / 32)) >>
...
#define SAM0_USB_IRQ_CONNECT(n)                                               \
	do {                                                                  \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(0, n, irq),                    \
			    DT_INST_IRQ_BY_IDX(0, n, priority), usb_sam0_isr, \
			    0, 0);                                            \
...
int usb_dc_ep_write(uint8_t ep, const uint8_t *buf, uint32_t len,
		    uint32_t *ret_bytes)
{
	struct usb_sam0_data *data = usb_sam0_get_data();
	UsbDevice *regs = &REGS->DEVICE;
	...
	uint32_t capacity =
		usb_sam0_pcksize_bytes[desc->DeviceDescBank[1].PCKSIZE.bit.SIZE];
	...
	endpoint->EPINTFLAG.reg = USB_DEVICE_EPINTFLAG_TRCPT1 |
...
int usb_dc_ep_mps(const uint8_t ep)
{
	...
	if (for_in) {
		...
		return usb_sam0_pcksize_bytes[desc->DeviceDescBank[1]
						      .PCKSIZE.bit.SIZE];
	} else {
		/* if endpoint is not configured, this should return 0 */
		...
		return usb_sam0_pcksize_bytes[desc->DeviceDescBank[0]
						      .PCKSIZE.bit.SIZE];
	}
}
