				...
				const uint8_t *tx_data, int len)
{
	...
	while ((len - num_tx > 0) && (LPSCI_GetStatusFlags(config->base) &
				      kLPSCI_TxDataRegEmptyFlag)) {
				...
				const int len)
{
	...
	while ((len - num_rx > 0) && (LPSCI_GetStatusFlags(config->base) &
...
{
	return (mcux_lpsci_irq_tx_ready(dev) || mcux_lpsci_irq_rx_ready(dev));
}
...
#define MCUX_LPSCI_INIT(n)                                                    \
	...
	static const struct mcux_lpsci_config mcux_lpsci_##n##_config;        \
                                                                              \
	DEVICE_AND_API_INIT(uart_##n, DT_INST_LABEL(n), &mcux_lpsci_init,     \
			    &mcux_lpsci_##n##_data, &mcux_lpsci_##n##_config, \
