...
#define PL011_RSR_ERROR_MASK                                      \
	(PL011_RSR_ECR_FE | PL011_RSR_ECR_PE | PL011_RSR_ECR_BE | \
	 PL011_RSR_ECR_OE)
...
#define PL011_IMSC_MASK_ALL                                        \
	(PL011_IMSC_OEIM | PL011_IMSC_BEIM | PL011_IMSC_PEIM |     \
	 PL011_IMSC_FEIM | PL011_IMSC_RIMIM | PL011_IMSC_CTSMIM |  \
	 PL011_IMSC_DCDMIM | PL011_IMSC_DSRMIM | PL011_IMSC_RXIM | \
	 PL011_IMSC_TXIM | PL011_IMSC_RTIM)

#define DEV_CFG(dev) ((const struct uart_device_config *const)(dev)->config)
#define DEV_DATA(dev) ((struct pl011_data *)(dev)->data)
#define PL011_REGS(dev) ((volatile struct pl011_regs *)(DEV_CFG(dev))->base)
...
static int pl011_set_baudrate(const struct device *dev, uint32_t clk,
			      uint32_t baudrate)
{
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int pl011_fifo_fill(const struct device *dev, const uint8_t *tx_data,
			   int len)
{
	uint8_t num_tx = 0U;

	while (!(PL011_REGS(dev)->fr & PL011_FR_TXFF) && (len - num_tx > 0)) {
		PL011_REGS(dev)->dr = tx_data[num_tx++];
	}
	return num_tx;
}

static int pl011_fifo_read(const struct device *dev, uint8_t *rx_data,
			   const int len)
{
	uint8_t num_rx = 0U;

	while ((len - num_rx > 0) && !(PL011_REGS(dev)->fr & PL011_FR_RXFE)) {
...
{
	PL011_REGS(dev)->imsc |= PL011_IMSC_RXIM | PL011_IMSC_RTIM;
}
...
DEVICE_AND_API_INIT(pl011_port_0, DT_INST_LABEL(0), &pl011_init,
		    &pl011_data_port_0, &pl011_cfg_port_0, PRE_KERNEL_1,
		    CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &pl011_driver_api);

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static void pl011_irq_config_func_0(const struct device *dev)
{
#if DT_NUM_IRQS(DT_INST(0, arm_pl011)) == 1
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority), pl011_isr,
		    DEVICE_GET(pl011_port_0), 0);
	irq_enable(DT_INST_IRQN(0));
#else
	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, tx, irq),
		    DT_INST_IRQ_BY_NAME(0, tx, priority), pl011_isr,
		    DEVICE_GET(pl011_port_0), 0);
	irq_enable(DT_INST_IRQ_BY_NAME(0, tx, irq));

	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, rx, irq),
		    DT_INST_IRQ_BY_NAME(0, rx, priority), pl011_isr,
		    DEVICE_GET(pl011_port_0), 0);
	irq_enable(DT_INST_IRQ_BY_NAME(0, rx, irq));

	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, rxtim, irq),
		    DT_INST_IRQ_BY_NAME(0, rxtim, priority), pl011_isr,
		    DEVICE_GET(pl011_port_0), 0);
...
DEVICE_AND_API_INIT(pl011_port_1, DT_INST_LABEL(1), &pl011_init,
		    &pl011_data_port_1, &pl011_cfg_port_1, PRE_KERNEL_1,
		    CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &pl011_driver_api);
...
static void pl011_irq_config_func_1(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(1), DT_INST_IRQ(1, priority), pl011_isr,
		    DEVICE_GET(pl011_port_1), 0);
	irq_enable(DT_INST_IRQN(1));
#else
	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(1, tx, irq),
		    DT_INST_IRQ_BY_NAME(1, tx, priority), pl011_isr,
		    DEVICE_GET(pl011_port_1), 0);
	irq_enable(DT_INST_IRQ_BY_NAME(1, tx, irq));

	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(1, rx, irq),
		    DT_INST_IRQ_BY_NAME(1, rx, priority), pl011_isr,
		    DEVICE_GET(pl011_port_1), 0);
	irq_enable(DT_INST_IRQ_BY_NAME(1, rx, irq));

	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(1, rxtim, irq),
		    DT_INST_IRQ_BY_NAME(1, rxtim, priority), pl011_isr,
		    DEVICE_GET(pl011_port_1), 0);
