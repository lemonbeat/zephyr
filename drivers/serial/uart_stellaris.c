...
#define DEV_DATA(dev) ((struct uart_stellaris_dev_data_t *const)(dev)->data)
#define UART_STRUCT(dev) ((volatile struct _uart *)(DEV_CFG(dev))->base)
 ...
 * @return N/A
 */
static void baudrate_set(const struct device *dev, uint32_t baudrate,
...
{
	disable(dev);
	baudrate_set(dev, DEV_DATA(dev)->baud_rate, DEV_CFG(dev)->sys_clk_freq);
 ...
 */
static int uart_stellaris_fifo_read(const struct device *dev, uint8_t *rx_data,
				    const int size)
{
	volatile struct _uart *uart = UART_STRUCT(dev);
	uint8_t num_rx = 0U;
...
static void uart_stellaris_irq_err_enable(const struct device *dev)
{
	...
	uart->im |= (UARTTIM_RTIM | UARTTIM_FEIM | UARTTIM_PEIM | UARTTIM_BEIM |
		     UARTTIM_OEIM);
}
...
DEVICE_AND_API_INIT(uart_stellaris0, DT_INST_LABEL(0), &uart_stellaris_init,
...
static void irq_config_func_0(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
		    uart_stellaris_isr, DEVICE_GET(uart_stellaris0), 0);
...
DEVICE_AND_API_INIT(uart_stellaris1, DT_INST_LABEL(1), &uart_stellaris_init,
...
static void irq_config_func_1(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(1), DT_INST_IRQ(1, priority),
		    uart_stellaris_isr, DEVICE_GET(uart_stellaris1), 0);
...
DEVICE_AND_API_INIT(uart_stellaris2, DT_INST_LABEL(2), &uart_stellaris_init,
...
static void irq_config_func_2(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(2), DT_INST_IRQ(2, priority),
		    uart_stellaris_isr, DEVICE_GET(uart_stellaris2), 0);
