...
#define DEV_CFG(dev) ((const struct imx_uart_config *const)(dev)->config)
#define UART_STRUCT(dev) ((UART_Type *)(DEV_CFG(dev))->base)
...
static int uart_imx_fifo_fill(const struct device *dev, const uint8_t *tx_data,
			      int size)
{
	UART_Type *uart = UART_STRUCT(dev);
...
#define UART_IMX_CONFIG_FUNC(n)                                        \
	...
	{                                                              \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), \
			    uart_imx_isr, DEVICE_GET(uart_##n), 0);    \
