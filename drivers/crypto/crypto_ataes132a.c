				  ...
				  uint8_t *nresponse)
{
	...
	while (status & ATAES_STATUS_WIP) {
		k_busy_wait(D10D24S);
		read_reg_i2c(data->i2c, cfg->i2c_addr, ATAES_STATUS_REG,
			     &status);
	}
	...
	write_reg_i2c(data->i2c, cfg->i2c_addr, ATAES_COMMAND_ADDRR_RESET, 0x0);
	...
	/* Read the response */
	burst_read_i2c(data->i2c, cfg->i2c_addr, ATAES_COMMAND_MEM_ADDR,
			      ...
			      uint8_t *nonce_buf)
{
	 ...
	 * the nonce register.
	 */
	if (!(ataes132a_state[key_id].key_config &
	      ATAES_KEYCONFIG_RAND_NONCE) &&
	    nonce_buf) {
		param_buffer[0] = 0x0;
		param_buffer[1] = 0x0;
		param_buffer[2] = 0x0;
		...
		return_code = ataes132a_send_command(dev, ATAES_NONCE_OP, 0x0,
						     param_buffer, 16,
...
int ataes132a_aes_ecb_block(const struct device *dev, uint8_t key_id,
				   ...
				   enum cipher_op op_type)
{
	...
	read_reg_i2c(data->i2c, cfg->i2c_addr, ATAES_KEYCFG_REG(key_id),
		     &config);
	ataes132a_state[key_id].key_config = config;
	read_reg_i2c(data->i2c, cfg->i2c_addr, ATAES_CHIPCONFIG_REG, &config);
