...
static inline int rsc_stat(const struct device *dev, uint8_t set, uint8_t clear)
{
	...
	rc = i2c_write_read(data->i2c, cfg->addr, &addr, sizeof(addr),
...
static void validate_isw_monitoring(const struct device *dev)
{
	...
	if ((data->isw != NULL) && (isw_mon_req != data->isw_mon_req)) {
		...
		if ((rc >= 0) && ((isw_mon_req & ISW_MON_REQ_Sync) !=
...
static const uint8_t *decode_time(struct tm *tp, const uint8_t *rp,
				  bool with_sec)
{
	uint8_t reg;
			   ...
			   struct maxim_ds3231_alarm *cp)
{
	...
	rv = i2c_write_read(data->i2c, cfg->addr, &addr, sizeof(addr), rbp,
			    len);
...
static int set_alarm(const struct device *dev, uint8_t id,
...
static void counter_alarm_forwarder(const struct device *dev, uint8_t id,
...
static void alarm_worker(struct k_work *work)
{
	struct ds3231_data *data =
		CONTAINER_OF(work, struct ds3231_data, alarm_work);
	...
	while (af > 0) {
		...
		for (id = 0; id < cfg->generic.channels; ++id) {
			...
			if (data->alarm_flags[id] &
			    MAXIM_DS3231_ALARM_FLAGS_AUTODISABLE) {
...
{
	struct ds3231_data *data =
		CONTAINER_OF(work, struct ds3231_data, sqw_work);
...
static int read_time(const struct device *dev, time_t *time)
{
	...
	int rc = i2c_write_read(data->i2c, cfg->addr, &addr, sizeof(addr),
				&data->registers, 7);
...
static void sync_finish(const struct device *dev, int rc)
{
	...
	if (notify != NULL) {
		maxim_ds3231_notify_callback cb =
			(maxim_ds3231_notify_callback)sys_notify_finalize(
				notify, rc);
...
{
	struct ds3231_data *data =
		CONTAINER_OF(tmr, struct ds3231_data, sync_timer);
...
{
	struct ds3231_data *data =
		CONTAINER_OF(work, struct ds3231_data, sync_work);
	uint32_t syncclock = maxim_ds3231_read_syncclock(data->ds3231);
	bool unlock = true;
...
{
	struct ds3231_data *data =
		CONTAINER_OF(cb, struct ds3231_data, isw_callback);
...
static int ds3231_init(const struct device *dev)
{
	...
	rc = sc_ctrl(dev, MAXIM_DS3231_REG_CTRL_INTCN,
		     MAXIM_DS3231_REG_CTRL_RS_Msk | MAXIM_DS3231_ALARM1 |
			     MAXIM_DS3231_ALARM2);
	if (rc < 0) {
	...
	if (cfg->isw_gpios.ctrl != NULL) {
		const struct device *gpio =
			device_get_binding(cfg->isw_gpios.ctrl);
		...
		gpio_init_callback(&data->isw_callback, isw_gpio_callback,
...
DEVICE_AND_API_INIT(ds3231_0, DT_INST_LABEL(0), ds3231_init, &ds3231_0_data,
		    &ds3231_0_config, POST_KERNEL,
		    CONFIG_COUNTER_MAXIM_DS3231_INIT_PRIORITY, &ds3231_api);
				      ...
				      struct maxim_ds3231_syncpoint *syncpoint)
{
	...
	Z_OOPS(Z_SYSCALL_SPECIFIC_DRIVER(dev, K_OBJ_DRIVER_COUNTER,
					 &ds3231_api));
