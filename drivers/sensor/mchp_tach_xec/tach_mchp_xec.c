...
#define TACH_XEC_REG_BASE(_dev)                                           \
	((TACH_Type *)((const struct tach_xec_config *const)_dev->config) \
		 ->base_address)

#define TACH_XEC_CONFIG(_dev) \
	(((const struct counter_xec_config *const)_dev->config))

#define TACH_XEC_DATA(_dev) ((struct tach_xec_data *)dev->data)
...
int tach_xec_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	while (poll_count < PIN_STS_TIMEOUT) {
		...
		if (tach->STATUS & MCHP_TACH_STS_CNT_RDY) {
			data->count = tach->CONTROL >>
				      MCHP_TACH_CTRL_COUNTER_POS;
			break;
		}
...
#define TACH_XEC_DEVICE(id)                                                   \
	static const struct tach_xec_config tach_xec_dev_config##id = {       \
		.base_address = DT_INST_REG_ADDR(id),                         \
	};                                                                    \
                                                                              \
	static struct tach_xec_data tach_xec_dev_data##id;                    \
                                                                              \
	DEVICE_AND_API_INIT(tach##id, DT_INST_LABEL(id), tach_xec_init,       \
			    &tach_xec_dev_data##id, &tach_xec_dev_config##id, \
