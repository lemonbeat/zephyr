...
static const uint16_t lsm6dsl_odr_map[] = { 0,	 12,  26,   52,	  104, 208,
...
static int lsm6dsl_accel_set_fs_raw(const struct device *dev, uint8_t fs)
{
	...
	if (data->hw_tf->update_reg(dev, LSM6DSL_REG_CTRL1_XL,
...
static int lsm6dsl_gyro_set_fs_raw(const struct device *dev, uint8_t fs)
{
	...
	if (fs == GYRO_FULLSCALE_125) {
		if (data->hw_tf->update_reg(
			    dev, LSM6DSL_REG_CTRL2_G, LSM6DSL_MASK_CTRL2_FS125,
			    1 << LSM6DSL_SHIFT_CTRL2_FS125) < 0) {
			return -EIO;
		}
	} else {
		if (data->hw_tf->update_reg(
			    dev, LSM6DSL_REG_CTRL2_G, LSM6DSL_MASK_CTRL2_G_FS_G,
...
static int lsm6dsl_accel_range_set(const struct device *dev, int32_t range)
{
	...
	data->accel_sensitivity =
		(float)(lsm6dsl_accel_fs_sens[fs] * SENSI_GRAIN_XL);
	return 0;
}
#endif
...
static int lsm6dsl_gyro_range_set(const struct device *dev, int32_t range)
{
	...
	data->gyro_sensitivity =
		(float)(lsm6dsl_gyro_fs_sens[fs] * SENSI_GRAIN_G);
	return 0;
}
#endif
...
static int lsm6dsl_sample_fetch_accel(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(dev, LSM6DSL_REG_OUTX_L_XL, buf,
				   sizeof(buf)) < 0) {
...
static int lsm6dsl_sample_fetch_gyro(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(dev, LSM6DSL_REG_OUTX_L_G, buf,
				   sizeof(buf)) < 0) {
		LOG_DBG("failed to read sample");
		return -EIO;
	}
...
static int lsm6dsl_sample_fetch_temp(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(dev, LSM6DSL_REG_OUT_TEMP_L, buf,
				   sizeof(buf)) < 0) {
		LOG_DBG("failed to read sample");
		return -EIO;
	}
...
static int lsm6dsl_sample_fetch_press(const struct device *dev)
{
	...
	data->sample_press =
		(int32_t)((uint32_t)(buf[0]) | ((uint32_t)(buf[1]) << 8) |
			  ((uint32_t)(buf[2]) << 16));
	data->sample_temp =
		(int16_t)((uint16_t)(buf[3]) | ((uint16_t)(buf[4]) << 8));
					   ...
					   struct lsm6dsl_data *data)
{
	...
	case SENSOR_CHAN_MAGN_X:
		lsm6dsl_magn_convert(val, data->magn_sample_x,
				     data->magn_sensitivity);
		break;
	case SENSOR_CHAN_MAGN_Y:
		lsm6dsl_magn_convert(val, data->magn_sample_y,
				     data->magn_sensitivity);
		break;
	case SENSOR_CHAN_MAGN_Z:
		lsm6dsl_magn_convert(val, data->magn_sample_z,
				     data->magn_sensitivity);
		break;
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6dsl_magn_convert(val, data->magn_sample_x,
				     data->magn_sensitivity);
		lsm6dsl_magn_convert(val + 1, data->magn_sample_y,
				     data->magn_sensitivity);
		lsm6dsl_magn_convert(val + 2, data->magn_sample_z,
				     data->magn_sensitivity);
		break;
	default:
		return -ENOTSUP;
...
static int lsm6dsl_init_chip(const struct device *dev)
{
	...
	if (data->hw_tf->update_reg(dev, LSM6DSL_REG_FIFO_CTRL5,
				    LSM6DSL_MASK_FIFO_CTRL5_FIFO_MODE,
				    0 << LSM6DSL_SHIFT_FIFO_CTRL5_FIFO_MODE) <
	    0) {
...
#define LSM6DSL_DEVICE_INIT(inst)                                              \
	DEVICE_AND_API_INIT(lsm6dsl_##inst, DT_INST_LABEL(inst), lsm6dsl_init, \
			    &lsm6dsl_data_##inst, &lsm6dsl_config_##inst,      \
...
#define LSM6DSL_DATA_SPI(inst) \
	COND_CODE_1(LSM6DSL_HAS_CS(inst), (LSM6DSL_DATA_SPI_CS(inst)), ({}))

#define LSM6DSL_SPI_CS_PTR(inst)                                            \
	COND_CODE_1(LSM6DSL_HAS_CS(inst), (&(lsm6dsl_data_##inst.cs_ctrl)), \
		    (NULL))
...
#define LSM6DSL_CONFIG_SPI(inst)                           \
	{                                                  \
		.bus_name = DT_INST_BUS_LABEL(inst),       \
		.bus_init = lsm6dsl_spi_init, .bus_cfg = { \
			.spi_cfg = LSM6DSL_SPI_CFG(inst)   \
		}                                          \
	}
...
#define LSM6DSL_CONFIG_I2C(inst)                                \
	{                                                       \
		.bus_name = DT_INST_BUS_LABEL(inst),            \
		.bus_init = lsm6dsl_i2c_init, .bus_cfg = {      \
...
#define LSM6DSL_DEFINE(inst)                                               \
	COND_CODE_1(DT_INST_ON_BUS(inst, spi), (LSM6DSL_DEFINE_SPI(inst)), \
