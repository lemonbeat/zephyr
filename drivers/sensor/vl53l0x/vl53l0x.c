				...
				enum sensor_channel chan)
{
	...
	ret = VL53L0X_PerformSingleRangingMeasurement(
		&drv_data->vl53l0x, &drv_data->RangingMeasurementData);
	if (ret < 0) {
			       ...
			       struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_PROX) {
		...
		val->val1 =
			drv_data->RangingMeasurementData.RangeMilliMeter / 1000;
		val->val2 = (drv_data->RangingMeasurementData.RangeMilliMeter %
			     1000) *
			    1000;
	}
...
static int vl53l0x_setup_single_shot(const struct device *dev)
{
	...
	ret = VL53L0X_PerformRefCalibration(&drv_data->vl53l0x, &VhvSettings,
					    &PhaseCal);
	if (ret) {
	...
	ret = VL53L0X_SetLimitCheckEnable(
		&drv_data->vl53l0x, VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, 1);
	if (ret) {
	...
	ret = VL53L0X_SetLimitCheckEnable(
		&drv_data->vl53l0x, VL53L0X_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE,
		1);
	if (ret) {
	...
	ret = VL53L0X_SetLimitCheckValue(
		&drv_data->vl53l0x, VL53L0X_CHECKENABLE_SIGNAL_RATE_FINAL_RANGE,
	...
	ret = VL53L0X_SetMeasurementTimingBudgetMicroSeconds(
	...
	ret = VL53L0X_SetVcselPulsePeriod(
		&drv_data->vl53l0x, VL53L0X_VCSEL_PERIOD_FINAL_RANGE,
...
static int vl53l0x_init(const struct device *dev)
{
	...
	if (gpio_pin_configure(gpio, DT_INST_GPIO_PIN(0, xshut_gpios),
	...
	ret = VL53L0X_RdWord(&drv_data->vl53l0x, VL53L0X_REG_WHO_AM_I,
			     (uint16_t *)&vl53l0x_id);
