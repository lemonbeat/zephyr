					...
					struct dps310_cal_coeff *comp)
{
	...
	comp->c00 = (((uint32_t)raw_coef[3]) << 12) +
		    (((uint16_t)raw_coef[4]) << 4) + (raw_coef[5] >> 4);
	/* coefficient is 2nd compliment */
				    ...
				    const struct dps310_cfg *config)
{
	...
	static const uint8_t psr_meas_cmd[] = {
		IFX_DPS310_REG_ADDR_MEAS_CFG, IFX_DPS310_MODE_COMMAND_PRESSURE
	};
...
static int dps310_init(const struct device *dev)
{
	...
	int res = i2c_reg_read_byte(data->i2c_master, config->i2c_addr,
				    IFX_DPS310_REG_ADDR_PRODUCT_ID,
				    &product_id);
...
DEVICE_AND_API_INIT(dps310, DT_INST_LABEL(0), dps310_init, &dps310_data_0,
		    &dps310_cfg_0, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,
		    &dps310_api_funcs);
#endif
...
DEVICE_AND_API_INIT(dps310, DT_INST_LABEL(1), dps310_init, &dps310_data_1,
		    &dps310_cfg_1, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,
		    &dps310_api_funcs);
#endif
