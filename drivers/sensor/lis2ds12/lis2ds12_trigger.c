...
static int lis2ds12_init_interrupt(const struct device *dev)
{
	...
	/* Enable latched mode */
	if (data->hw_tf->update_reg(data, LIS2DS12_REG_CTRL3, LIS2DS12_MASK_LIR,
	...
	if (data->hw_tf->update_reg(data, LIS2DS12_REG_CTRL4,
...
int lis2ds12_trigger_init(const struct device *dev)
{
	...
	gpio_init_callback(&data->gpio_cb, lis2ds12_gpio_callback,
			   BIT(cfg->irq_pin));
	...
	k_thread_create(&data->thread, data->thread_stack,
			...
			(k_thread_entry_t)lis2ds12_thread, data, NULL, NULL,
			K_PRIO_COOP(CONFIG_LIS2DS12_THREAD_PRIORITY), 0,
			K_NO_WAIT);
#elif defined(CONFIG_LIS2DS12_TRIGGER_GLOBAL_THREAD)
