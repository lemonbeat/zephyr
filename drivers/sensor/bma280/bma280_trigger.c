...
static inline void setup_int1(const struct device *dev, bool enable)
{
	...
	gpio_pin_interrupt_configure(
		data->gpio, DT_INST_GPIO_PIN(0, int1_gpios),
		    ...
		    enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if (attr == SENSOR_ATTR_SLOPE_TH) {
		...
		if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
					BMA280_REG_INT_5, BMA280_SLOPE_DUR_MASK,
		       ...
		       sensor_trigger_handler_t handler)
{
	...
	if (trig->type == SENSOR_TRIG_DATA_READY) {
		...
		if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
					BMA280_REG_INT_EN_1, BMA280_BIT_DATA_EN,
					0) < 0) {
...
int bma280_init_interrupt(const struct device *dev)
{
	...
	if (i2c_reg_write_byte(
		    drv_data->i2c, BMA280_I2C_ADDRESS, BMA280_REG_INT_RST_LATCH,
	...
	gpio_pin_configure(drv_data->gpio, DT_INST_GPIO_PIN(0, int1_gpios),
			   DT_INST_GPIO_FLAGS(0, int1_gpios) | GPIO_INPUT);
	...
	if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
				BMA280_REG_INT_MAP_1, BMA280_INT_MAP_1_BIT_DATA,
	...
	if (i2c_reg_update_byte(drv_data->i2c, BMA280_I2C_ADDRESS,
				BMA280_REG_INT_EN_0, BMA280_SLOPE_EN_XYZ,
				0) < 0) {
		LOG_DBG("Could not disable data ready interrupt");
		return -EIO;
	}
	...
	k_thread_create(&drv_data->thread, drv_data->thread_stack,
			...
			(k_thread_entry_t)bma280_thread, drv_data, NULL, NULL,
			K_PRIO_COOP(CONFIG_BMA280_THREAD_PRIORITY), 0,
			K_NO_WAIT);
#elif defined(CONFIG_BMA280_TRIGGER_GLOBAL_THREAD)
