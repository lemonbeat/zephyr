...
{
	uint32_t polynomial =
		(ENS210_CRC7_POLY << (ENS210_CRC7_DATA_WIDTH - 1));
			      ...
			      struct sensor_value *val)
{
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		...
		temp_frac =
			sys_le16_to_cpu(drv_data->temp.val) * (1000000 / 64);
		temp_frac -= 273150000;
...
static int ens210_sys_reset(const struct device *i2c_dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed to set SYS_CTRL to 0x%x",
			*(uint8_t *)&sys_ctrl);
	}
	return ret;
}
...
{
	const struct ens210_sys_ctrl sys_ctrl = { .low_power = low_power,
						  .reset = 0 };
	int ret;
...
{
	struct ens210_data *drv_data = dev->data;
	const struct ens210_sens_run sense_run = { .t_run = ENS210_T_RUN,
						   .h_run = ENS210_H_RUN };

#if defined(CONFIG_ENS210_TEMPERATURE_CONTINUOUS) || \
	defined(CONFIG_ENS210_HUMIDITY_CONTINUOUS)
	const struct ens210_sens_start sense_start = {
		.t_start = ENS210_T_RUN, .h_start = ENS210_H_RUN
	};
#endif
...
DEVICE_AND_API_INIT(ens210, DT_INST_LABEL(0), ens210_init, &ens210_driver, NULL,
