				   ...
				   enum sensor_channel chan)
{
	...
	if (i2c_burst_read(drv_data->i2c, config->i2c_address,
			   LSM303DLHC_REG_MAGN_X_LSB, magn_buf, 6) < 0) {
...
static void lsm303dlhc_convert(struct sensor_value *val, int64_t raw_val)
{
...
static int lsm303dlhc_magn_init(const struct device *dev)
{
	...
	if (drv_data->i2c == NULL) {
		LOG_ERR("Could not get pointer to %s device", config->i2c_name);
		return -ENODEV;
	}
...
DEVICE_AND_API_INIT(lsm303dlhc_magn, DT_INST_LABEL(0), lsm303dlhc_magn_init,
		    &lsm303dlhc_magn_driver, &lsm303dlhc_magn_config,
