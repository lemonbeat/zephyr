...
static uint32_t wwdg_stm32_get_timeout(const struct device *dev,
				       uint32_t prescaler, uint32_t counter)
{
...
static void wwdg_stm32_convert_timeout(const struct device *dev,
				       uint32_t timeout, uint32_t *prescaler,
				       uint32_t *counter)
{
	...
	*counter = 0;
	...
	for (divider = 8; divider >= 1; divider >>= 1) {
		wwdg_freq =
			((float)clock_freq) / WWDG_INTERNAL_DIVIDER / divider;
...
static struct wwdg_stm32_data wwdg_stm32_dev_data = { .counter =
							      WWDG_RESET_LIMIT,
						      .callback = NULL };

static struct wwdg_stm32_config wwdg_stm32_dev_config = {
	.pclken = { .enr = DT_INST_CLOCKS_CELL(0, bits),
		    .bus = DT_INST_CLOCKS_CELL(0, bus) },
	.Instance = (WWDG_TypeDef *)DT_INST_REG_ADDR(0),
};

DEVICE_AND_API_INIT(wwdg_stm32, DT_INST_LABEL(0), wwdg_stm32_init,
		    &wwdg_stm32_dev_data, &wwdg_stm32_dev_config, POST_KERNEL,
		    CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &wwdg_stm32_api);
