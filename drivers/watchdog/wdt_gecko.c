...
#define DEV_DATA(dev) ((struct wdt_gecko_data *)(dev)->data)
#define DEV_CFG(dev) ((const struct wdt_gecko_cfg *)(dev)->config)
...
static int wdt_gecko_setup(const struct device *dev, uint8_t options)
{
	...
	data->wdog_config.debugRun = (options & WDT_OPT_PAUSE_HALTED_BY_DBG) ==
				     0U;
				     ...
				     const struct wdt_timeout_cfg *cfg)
{
	...
	data->wdog_config.perSel =
		(WDOG_PeriodSel_TypeDef)wdt_gecko_get_persel_from_timeout(
			cfg->window.max);

	installed_timeout =
		wdt_gecko_get_timeout_from_persel(data->wdog_config.perSel);
	...
	if (cfg->window.min > 0) {
		 ...
		 */
		data->wdog_config.winSel =
			(WDOG_WinSel_TypeDef)wdt_gecko_convert_window(
...
#define GECKO_WDT_INIT(index)                                                  \
	...
	static struct wdt_gecko_data wdt_gecko_data_##index;                   \
                                                                               \
	DEVICE_AND_API_INIT(wdt_##index, DT_INST_LABEL(index),                 \
