...
#define PWM_PWMCR_SWR(x)                                        \
	(((uint32_t)(((uint32_t)(x)) << PWM_PWMCR_SWR_SHIFT)) & \
	 PWM_PWMCR_SWR_MASK)

#define DEV_CFG(dev) ((const struct imx_pwm_config *const)(dev)->config)
#define DEV_DATA(dev) ((struct imx_pwm_data *const)(dev)->data)
#define DEV_BASE(dev) ((PWM_Type *)(DEV_CFG(dev))->base)
			   ...
			   pwm_flags_t flags)
{
	...
	if (data->period_cycles != period_cycles) {
		LOG_WRN("Changing period cycles from %d to %d in %s",
			data->period_cycles, period_cycles, dev->name);
...
#define PWM_IMX_INIT(n)                                                      \
	...
	static struct imx_pwm_data imx_pwm_data_##n;                         \
                                                                             \
	DEVICE_AND_API_INIT(imx_pwm_##n, DT_INST_LABEL(n), &imx_pwm_init,    \
			    &imx_pwm_data_##n, &imx_pwm_config_##n,          \
