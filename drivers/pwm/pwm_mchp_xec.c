...
#define PWM_XEC_REG_BASE(_dev)                                          \
	((PWM_Type *)((const struct pwm_xec_config *const)_dev->config) \
		 ->base_address)

#define PWM_XEC_CONFIG(_dev) \
	(((const struct pwm_xec_config *const)_dev->config))
...
uint32_t max_freq_high_on_div[NUM_DIV_ELEMS] = {
	48000000, 24000000, 16000000, 12000000, 9600000, 8000000,
	6857142,  6000000,  5333333,  4800000,	4363636, 4000000,
	3692307,  3428571,  3200000,  3000000
};

uint32_t max_freq_low_on_div[NUM_DIV_ELEMS] = { 100000, 50000, 33333, 25000,
						20000,	16666, 14285, 12500,
						11111,	10000, 9090,  8333,
						7692,	7142,  6666,  6250 };
...
static uint16_t xec_compare_div_on_off(uint32_t target_freq, uint32_t dc,
				       uint32_t max_freq[16], uint8_t div_a,
				       uint8_t div_b, uint32_t *on_a,
				       uint32_t *off_a)
{
...
static uint8_t xec_select_best_div_on_off(uint32_t target_freq, uint32_t dc,
					  uint32_t max_freq[16], uint32_t *on,
					  uint32_t *off)
{
	int div_comp;
	uint8_t div;
	...
	for (div_comp = (int)div - 1; div_comp >= 0; div_comp--) {
		div = xec_compare_div_on_off(target_freq, dc, max_freq, div,
					     div_comp, on, off);
	}
					     ...
					     struct xec_params *lc_params)
{
	...
	if (hc_params->div < NUM_DIV_ELEMS) {
		freq_h = xec_compute_frequency(
			max_freq_high_on_div[hc_params->div], hc_params->on,
			hc_params->off);
	}

	if (lc_params->div < NUM_DIV_ELEMS) {
		freq_l = xec_compute_frequency(
			max_freq_low_on_div[lc_params->div], lc_params->on,
			lc_params->off);
	}
	...
	LOG_DBG("\tOn %s clock, ON %u OFF %u DIV %u",
		params == hc_params ? "High" : "Low", params->on, params->off,
		params->div);
...
static void xec_compute_and_set_parameters(const struct device *dev,
					   uint32_t target_freq, uint32_t on,
					   uint32_t off)
{
	...
	LOG_DBG("Target freq (x%u): %u and DC %u per-cent", XEC_PWM_FREQ_PF,
		target_freq, (dc / 1000));
	...
	if (compute_high) {
		...
		hc_params.div = xec_select_best_div_on_off(target_freq, dc,
							   max_freq_high_on_div,
							   &hc_params.on,
							   &hc_params.off);
		LOG_DBG("Best div high: %u (on/off: %u/%u)", hc_params.div,
	...
	if (compute_low) {
		lc_params.div = xec_select_best_div_on_off(target_freq, dc,
							   max_freq_low_on_div,
							   &lc_params.on,
							   &lc_params.off);
		LOG_DBG("Best div low: %u (on/off: %u/%u)", lc_params.div,
...
#define XEC_INST_INIT(inst)                                                    \
	static struct pwm_xec_config pwm_xec_dev_config_##inst = {             \
		.base_address = DT_INST_REG_ADDR(inst)                         \
                                                                               ...
                                                                               \
	DEVICE_AND_API_INIT(pwm_xec_##inst, DT_INST_LABEL(inst), pwm_xec_init, \
			    NULL, &pwm_xec_dev_config_##inst, POST_KERNEL,     \
			    CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &pwm_xec_api);
