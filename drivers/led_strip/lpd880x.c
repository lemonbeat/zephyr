...
static int lpd880x_strip_init(const struct device *dev)
{
	...
	if (!data->spi) {
		LOG_ERR("SPI device %s not found", DT_INST_BUS_LABEL(0));
		return -ENODEV;
	}
	...
	config->slave =
		DT_INST_REG_ADDR(0); /* MOSI/CLK only; CS is not supported. */
	config->cs = NULL;
...
DEVICE_AND_API_INIT(lpd880x_strip, DT_INST_LABEL(0), lpd880x_strip_init,
		    &lpd880x_strip_data, NULL, POST_KERNEL,
