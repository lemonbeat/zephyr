...
static int lp503x_set_brightness(const struct device *dev, uint32_t led,
				 uint8_t value)
{
	const struct lp503x_config *config = DEV_CFG(dev);
				 ...
				 const uint8_t *buf)
{
	...
	return i2c_write(data->i2c, data->chan_buf, num_channels + 1,
			 config->i2c_addr);
}
...
static int lp503x_init(const struct device *dev)
{
	...
	if (config->num_leds > LP503X_MAX_LEDS) {
		LOG_ERR("%s: invalid number of LEDs %d (max %d)", dev->name,
			config->num_leds, LP503X_MAX_LEDS);
		return -EINVAL;
	}
...
#define LP503x_DEVICE(id)                                                     \
	...
	DT_INST_FOREACH_CHILD(id, COLOR_MAPPING)                              \
                                                                              \
	const struct led_info lp503x_leds_##id[] = { DT_INST_FOREACH_CHILD(   \
		id, LED_INFO) };                                              \
                                                                              \
	...
	static struct lp503x_data lp503x_data_##id = {                        \
		.chan_buf = lp503x_chan_buf_##id,                             \
                                                                              ...
                                                                              \
	DEVICE_AND_API_INIT(lp503x_led_##id, DT_INST_LABEL(id), &lp503x_init, \
			    &lp503x_data_##id, &lp503x_config_##id,           \
