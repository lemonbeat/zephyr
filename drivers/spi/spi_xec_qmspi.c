...
static void qmspi_set_signalling_mode(QMSPI_Type *regs, uint32_t smode)
{
	...
	ptbl = smode_tbl;
	if (((regs->MODE >> MCHP_QMSPI_M_FDIV_POS) & MCHP_QMSPI_M_FDIV_MASK0) ==
	    1) {
		ptbl = smode48_tbl;
	}
...
#define XEC_QMSPI_CS_TIMING_VAL(a, b, c, d)                    \
	(((a)&0xFu) | (((b)&0xFu) << 8) | (((c)&0xFu) << 16) | \
	 (((d)&0xFu) << 24))

#define XEC_QMSPI_0_CS_TIMING                              \
	XEC_QMSPI_CS_TIMING_VAL(DT_INST_PROP(0, dcsckon),  \
...
DEVICE_AND_API_INIT(spi_xec_qmspi_0, DT_INST_LABEL(0), &qmspi_init,
		    &spi_qmspi_0_dev_data, &spi_qmspi_0_config, POST_KERNEL,
