...
static void prepare_for_transfer(const struct device *dev)
{
	...
	if (buf_len > 0) {
		...
		result = nrfx_spis_buffers_set(
			&dev_config->spis, ctx->tx_buf,
			spi_context_tx_buf_on(ctx) ? buf_len : 0, ctx->rx_buf,
...
static int init_spis(const struct device *dev, const nrfx_spis_config_t *config)
{
	struct spi_nrfx_data *dev_data = get_dev_data(dev);
	 ...
	 */
	nrfx_err_t result = nrfx_spis_init(&get_dev_config(dev)->spis, config,
...
#define SPI_NRFX_SPIS_DEVICE(idx)                                             \
	...
	DEVICE_AND_API_INIT(spi_##idx, DT_LABEL(SPIS(idx)), spi_##idx##_init, \
			    &spi_##idx##_data, &spi_##idx##z_config,          \
