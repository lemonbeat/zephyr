...
#define DEV_CFG(dev) ((const struct can_stm32_config *const)(dev)->config)
...
#define CAN_BANK_IS_EMPTY(usage, bank_nr) \
	(((usage >> ((bank_nr)*4)) & 0x0F) == 0x0F)
#define CAN_BANK_IN_LIST_MODE(can, bank) ((can)->FM1R & (1U << (bank)))
#define CAN_BANK_IN_32BIT_MODE(can, bank) ((can)->FS1R & (1U << (bank)))
#define CAN_IN_16BIT_LIST_MODE(can, bank) \
	(CAN_BANK_IN_LIST_MODE(can, bank) && !CAN_BANK_IN_32BIT_MODE(can, bank))
#define CAN_IN_16BIT_MASK_MODE(can, bank)     \
	(!CAN_BANK_IN_LIST_MODE(can, bank) && \
	 !CAN_BANK_IN_32BIT_MODE(can, bank))
#define CAN_IN_32BIT_LIST_MODE(can, bank) \
	(CAN_BANK_IN_LIST_MODE(can, bank) && CAN_BANK_IN_32BIT_MODE(can, bank))
#define CAN_IN_32BIT_MASK_MODE(can, bank) \
	(!CAN_BANK_IN_LIST_MODE(can, bank) && CAN_BANK_IN_32BIT_MODE(can, bank))
struct can_mailbox {
