			   ...
			   struct can_loopback_filter *filter)
{
	...
	LOG_DBG("Receiving %d bytes. Id: 0x%x, ID type: %s %s", frame->dlc,
...
int can_loopback_send(const struct device *dev, const struct zcan_frame *frame,
...
static int can_loopback_init(const struct device *dev)
{
	...
	tx_tid = k_thread_create(&tx_thread_data, tx_thread_stack,
				 ...
				 CONFIG_CAN_LOOPBACK_TX_THREAD_PRIORITY, 0,
				 K_NO_WAIT);
	if (!tx_tid) {
