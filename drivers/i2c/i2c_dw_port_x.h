...
DEVICE_AND_API_INIT(i2c_ @NUM @, DT_INST_LABEL(@NUM @), &i2c_dw_initialize,
		    &i2c_ @NUM @_runtime, &i2c_config_dw_ @NUM @, POST_KERNEL,
...
static void i2c_config_ @NUM @(const struct device *port)
{
	...
	irq_connect_dynamic(irq, DT_INST_IRQ(@NUM @, priority),
	...
	IRQ_CONNECT(DT_INST_IRQN(@NUM @), DT_INST_IRQ(@NUM @, priority),
		    i2c_dw_isr, DEVICE_GET(i2c_ @NUM @),
		    INST_ @NUM @_IRQ_FLAGS);
	pcie_irq_enable(DT_INST_REG_ADDR(@NUM @), DT_INST_IRQN(@NUM @));
