...
static int i2c_sifive_send_addr(const struct device *dev, uint16_t addr,
				uint16_t rw_flag)
{
	const struct i2c_sifive_cfg *config = dev->config;
	uint8_t command = 0U;
...
static int i2c_sifive_write_msg(const struct device *dev, struct i2c_msg *msg,
				uint16_t addr)
{
	const struct i2c_sifive_cfg *config = dev->config;
	int rc = 0;
...
static int i2c_sifive_read_msg(const struct device *dev, struct i2c_msg *msg,
			       uint16_t addr)
{
	const struct i2c_sifive_cfg *config = dev->config;
	uint8_t command = 0U;
...
static int i2c_sifive_configure(const struct device *dev, uint32_t dev_config)
{
	...
	sys_write8((uint8_t)(0xFF & prescale),
		   I2C_REG(config, REG_PRESCALE_LOW));
...
static int i2c_sifive_transfer(const struct device *dev, struct i2c_msg *msgs,
...
#define I2C_SIFIVE_INIT(n)                                                    \
	...
	DEVICE_AND_API_INIT(i2c_##n, DT_INST_LABEL(n), i2c_sifive_init, NULL, \
			    &i2c_sifive_cfg_##n, POST_KERNEL,                 \
			    CONFIG_I2C_INIT_PRIORITY, &i2c_sifive_api);
