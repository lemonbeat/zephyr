...
#define I2C_LITEX_INIT(n)                                                      \
	static const struct i2c_litex_cfg i2c_litex_cfg_##n = {                \
		.w_reg = (volatile uint32_t *)DT_INST_REG_ADDR_BY_NAME(n,      \
								       write), \
		.r_reg = (volatile uint32_t *)DT_INST_REG_ADDR_BY_NAME(n,      \
								       read),  \
	};                                                                     \
                                                                               \
	static struct i2c_bitbang i2c_bitbang_##n;                             \
                                                                               \
	DEVICE_AND_API_INIT(litex_i2c_##n, DT_INST_LABEL(n), i2c_litex_init,   \
			    &i2c_bitbang_##n, &i2c_litex_cfg_##n, POST_KERNEL, \
