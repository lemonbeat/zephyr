...
static inline int ssd1306_write_bus(const struct device *dev, uint8_t *buf,
...
{
	uint8_t cmd_buf[] = { (SSD1306_PANEL_SEGMENT_REMAP ?
...
static inline int ssd1306_set_timing_setting(const struct device *dev)
{
	uint8_t cmd_buf[] = { SSD1306_SET_CLOCK_DIV_RATIO,
			      (SSD1306_CLOCK_FREQUENCY << 4) |
				      SSD1306_CLOCK_DIV_RATIO,
...
{
	uint8_t cmd_buf[] = {
		SSD1306_SET_START_LINE,		 SSD1306_SET_DISPLAY_OFFSET,
		DT_INST_PROP(0, display_offset), SSD1306_SET_PADS_HW_CONFIG,
		SSD1306_COM_PINS_HW_CONFIG,	 SSD1306_SET_MULTIPLEX_RATIO,
...
static int ssd1306_write(const struct device *dev, const uint16_t x,
			 const uint16_t y,
			 ...
			 const void *buf)
{
	...
	LOG_DBG("x %u, y %u, pitch %u, width %u, height %u, buf_len %u", x, y,
		desc->pitch, desc->width, desc->height, buf_len);
	...
	for (uint8_t n = 0; n < desc->height / 8; n++) {
		cmd_buf[sizeof(cmd_buf) - 1] = SSD1306_SET_PAGE_START_ADDRESS |
					       (n + (y / 8));
...
static int ssd1306_init_device(const struct device *dev)
{
	...
	gpio_pin_set(driver->reset, DT_INST_GPIO_PIN(0, reset_gpios), 1);
	k_sleep(K_MSEC(SSD1306_RESET_DELAY));
	gpio_pin_set(driver->reset, DT_INST_GPIO_PIN(0, reset_gpios), 0);
#endif
...
static int ssd1306_init(const struct device *dev)
{
...
#if DT_INST_NODE_HAS_PROP(0, reset_gpios)
	driver->reset = device_get_binding(DT_INST_GPIO_LABEL(0, reset_gpios));
	...
	gpio_pin_configure(driver->reset, DT_INST_GPIO_PIN(0, reset_gpios),
	...
	driver->data_cmd =
		device_get_binding(DT_INST_GPIO_LABEL(0, data_cmd_gpios));
	...
	gpio_pin_configure(
		driver->data_cmd, DT_INST_GPIO_PIN(0, data_cmd_gpios),
		GPIO_OUTPUT_INACTIVE | DT_INST_GPIO_FLAGS(0, data_cmd_gpios));
...
DEVICE_AND_API_INIT(ssd1306, DT_INST_LABEL(0), ssd1306_init, &ssd1306_driver,
