...
static int sdl_display_init(const struct device *dev)
{
	...
	disp_data->window = SDL_CreateWindow(
		"Zephyr Display", SDL_WINDOWPOS_UNDEFINED,
	...
	disp_data->renderer = SDL_CreateRenderer(disp_data->window, -1,
	...
	disp_data->texture = SDL_CreateTexture(disp_data->renderer,
...
static void
sdl_display_write_argb8888(void *disp_buf,
...
static void
sdl_display_write_rgb888(uint8_t *disp_buf,
			 const struct display_buffer_descriptor *desc,
			 const void *buf)
{
	uint32_t w_idx;
...
static void
sdl_display_write_rgb565(uint8_t *disp_buf,
			 const struct display_buffer_descriptor *desc,
			 const void *buf)
{
	uint32_t w_idx;
...
static void
sdl_display_write_bgr565(uint8_t *disp_buf,
			 const struct display_buffer_descriptor *desc,
			 const void *buf)
{
	uint32_t w_idx;
...
static void
sdl_display_get_capabilities(const struct device *dev,
...
static int
sdl_display_set_pixel_format(const struct device *dev,
