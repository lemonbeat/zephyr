...
#define OVF_FLAG_ADDR     \
	(struct reg_addr) \
	{                 \
		0, 39     \
	}

#define PROC_BLK_SEL_ADDR \
	(struct reg_addr) \
	{                 \
		0, 60     \
	}
...
#define L_DIG_VOL_CTRL_ADDR \
	(struct reg_addr)   \
	{                   \
		0, 65       \
	}
#define DRC_CTRL1_ADDR    \
	(struct reg_addr) \
	{                 \
		0, 68     \
	}
#define L_BEEP_GEN_ADDR   \
	(struct reg_addr) \
	{                 \
		0, 71     \
	}
#define BEEP_GEN_EN_BEEP (BIT(7))
#define R_BEEP_GEN_ADDR   \
	(struct reg_addr) \
	{                 \
		0, 72     \
	}
#define BEEP_LEN_MSB_ADDR \
	(struct reg_addr) \
	{                 \
		0, 73     \
	}
#define BEEP_LEN_MIB_ADDR \
	(struct reg_addr) \
	{                 \
		0, 74     \
	}
#define BEEP_LEN_LSB_ADDR \
	(struct reg_addr) \
	{                 \
		0, 75     \
	}
...
#define HPL_ANA_VOL_CTRL_ADDR \
	(struct reg_addr)     \
	{                     \
		1, 36         \
	}
#define HPR_ANA_VOL_CTRL_ADDR \
	(struct reg_addr)     \
	{                     \
		1, 37         \
	}
...
#define HPX_ANA_VOL(val) (((val)&HPX_ANA_VOL_MASK) | HPX_ANA_VOL_ENABLE)
...
#define HPL_DRV_GAIN_CTRL_ADDR \
	(struct reg_addr)      \
	{                      \
		1, 40          \
	}
#define HPR_DRV_GAIN_CTRL_ADDR \
	(struct reg_addr)      \
	{                      \
		1, 41          \
	}
...
#define HEADPHONE_DRV_CTRL_ADDR \
	(struct reg_addr)       \
	{                       \
		1, 44           \
	}
