...
static ALWAYS_INLINE unsigned int arch_irq_lock(void)
{
	...
	__asm__ volatile("rdctl %[key], status\n\t"
			 "movi %[tmp], -2\n\t"
			 "and %[tmp], %[key], %[tmp]\n\t"
			 "wrctl status, %[tmp]\n\t"
			 : [key] "=r"(key), [tmp] "=r"(tmp)
...
#if (ALT_CPU_NUM_OF_SHADOW_REG_SETS > 0) || (defined ALT_CPU_EIC_PRESENT) || \
	(defined ALT_CPU_MMU_PRESENT) || (defined ALT_CPU_MPU_PRESENT)
	__asm__ volatile("andi %[key], %[key], 1\n\t"
			 "beq %[key], zero, 1f\n\t"
			 "rdctl %[key], status\n\t"
			 "ori %[key], %[key], 1\n\t"
			 "wrctl status, %[key]\n\t"
			 ...
			 : "memory");
#else
	__asm__ volatile("wrctl status, %[key]" : : [key] "r"(key) : "memory");
#endif
}
