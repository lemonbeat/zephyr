...
#define MPU_REGION_READ                                          \
	((UM_READ << BM0_UM_SHIFT) | (UM_READ << BM1_UM_SHIFT) | \
	 (UM_READ << BM2_UM_SHIFT) | (UM_READ << BM3_UM_SHIFT))

/* Write Attribute */
#define MPU_REGION_WRITE                                           \
	((UM_WRITE << BM0_UM_SHIFT) | (UM_WRITE << BM1_UM_SHIFT) | \
	 (UM_WRITE << BM2_UM_SHIFT) | (UM_WRITE << BM3_UM_SHIFT))

/* Execute Attribute */
#define MPU_REGION_EXEC                                          \
	((UM_EXEC << BM0_UM_SHIFT) | (UM_EXEC << BM1_UM_SHIFT) | \
	 (UM_EXEC << BM2_UM_SHIFT) | (UM_EXEC << BM3_UM_SHIFT))
...
#define REGION_RAM_ATTR                                                        \
	{                                                                      \
		((MPU_REGION_SU_RW) | ((UM_READ | UM_WRITE) << BM3_UM_SHIFT) | \
		 (BM4_PERMISSIONS))                                            \
	}
...
#define REGION_IO_ATTR                                                  \
	{                                                               \
		(MPU_REGION_READ | MPU_REGION_WRITE | MPU_REGION_EXEC | \
		 MPU_REGION_SU)                                         \
	}

#define REGION_RO_ATTR                            \
	{                                         \
...
#define REGION_DEBUGGER_AND_DEVICE_ATTR                                     \
	{                                                                   \
		((MPU_REGION_SU) | ((UM_READ | UM_WRITE) << BM3_UM_SHIFT) | \
		 (BM4_PERMISSIONS))                                         \
	}

#define REGION_DEBUG_ATTR     \
	{                     \
		MPU_REGION_SU \
	}

#define REGION_BACKGROUND_ATTR   \
	{                        \
		MPU_REGION_SU_RW \
	}
...
#define K_MEM_PARTITION_P_RW_U_RO \
	((k_mem_partition_attr_t){ (MPU_REGION_READ | MPU_REGION_SU_RW) })
#define K_MEM_PARTITION_P_RW_U_NA \
	((k_mem_partition_attr_t){ (MPU_REGION_SU_RW) })
#define K_MEM_PARTITION_P_RO_U_RO \
	((k_mem_partition_attr_t){ (MPU_REGION_READ | MPU_REGION_SU) })
#define K_MEM_PARTITION_P_RO_U_NA \
	((k_mem_partition_attr_t){ (MPU_REGION_SU_RX) })

/* Execution-allowed attributes */
#define K_MEM_PARTITION_P_RWX_U_RWX                                      \
	((k_mem_partition_attr_t){ (MPU_REGION_READ | MPU_REGION_WRITE | \
...
#define MPU_REGION_ENTRY(_name, _base, _end, _attr)                       \
	{                                                                 \
		.name = _name, .base = _base, .end = _end, .attr = _attr, \
	}
