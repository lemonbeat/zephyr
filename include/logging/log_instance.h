...
#define LOG_INSTANCE_PTR_DECLARE(_name) struct log_source_dynamic_data *_name

#define LOG_INSTANCE_REGISTER(_module_name, _inst_name, _level)                 \
	Z_LOG_CONST_ITEM_REGISTER(LOG_INSTANCE_FULL_NAME(_module_name,          \
							 _inst_name),           \
				  STRINGIFY(_module_name._inst_name), _level);  \
	struct log_source_dynamic_data LOG_INSTANCE_DYNAMIC_DATA(_module_name,  \
								 _inst_name)    \
