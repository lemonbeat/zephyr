...
#define LOG_FUNCTION_PREFIX_MASK                                \
	(((uint32_t)IS_ENABLED(CONFIG_LOG_FUNC_NAME_PREFIX_ERR) \
	 ...
	 ((uint32_t)IS_ENABLED(CONFIG_LOG_FUNC_NAME_PREFIX_INF) \
	  << LOG_LEVEL_INF) |                                   \
	 ((uint32_t)IS_ENABLED(CONFIG_LOG_FUNC_NAME_PREFIX_DBG) \
	  << LOG_LEVEL_DBG))
...
#define LOG_CONST_ID_GET(_addr)                                                \
	Z_LOG_EVAL(                                                            \
		...
		(__log_level ?                                                 \
			       log_const_source_id((                                 \
				 const struct log_source_const_data *)_addr) : \
			       0),                                                   \
		(0))
...
#define LOG_CURRENT_MODULE_ID() \
	(__log_level != 0 ? log_const_source_id(__log_current_const_data) : 0U)
...
#define LOG_CURRENT_DYNAMIC_DATA_ADDR()             \
	(__log_level ? __log_current_dynamic_data : \
			     (struct log_source_dynamic_data *)0U)
...
#define LOG_DYNAMIC_ID_GET(_addr)                                              \
	Z_LOG_EVAL(CONFIG_LOG,                                                 \
		   (__log_level ?                                              \
				  log_dynamic_source_id(                             \
				    (struct log_source_dynamic_data *)_addr) : \
				  0),                                                \
		   (0))
...
#define Z_LOG_STR(...)                                                    \
	"%s: " GET_ARG_N(1, __VA_ARGS__),                                 \
		__func__ COND_CODE_0(NUM_VA_ARGS_LESS_1(__VA_ARGS__), (), \
...
#define _LOG_NARGS_POSTFIX_IMPL(_ignored, _0, _1, _2, _3, _4, _5, _6, _7, _8, \
...
#define Z_LOG_NARGS_POSTFIX(...)                                             \
	_LOG_NARGS_POSTFIX_IMPL(__VA_ARGS__, LONG, LONG, LONG, LONG, LONG,   \
				LONG, LONG, LONG, LONG, LONG, LONG, LONG, 3, \
				2, 1, 0, ~)
...
#define _LOG_INTERNAL_0(_src_level, _str) log_0(_str, _src_level)
...
#define _LOG_INTERNAL_3(_src_level, _str, _arg0, _arg1, _arg2) \
	log_3(_str, (log_arg_t)(_arg0), (log_arg_t)(_arg1),    \
	      (log_arg_t)(_arg2), _src_level)
...
#define Z_LOG_CONST_LEVEL_CHECK(_level)                        \
	(IS_ENABLED(CONFIG_LOG) &&                             \
	 (Z_LOG_LEVEL_CHECK(_level, CONFIG_LOG_OVERRIDE_LEVEL, \
			    LOG_LEVEL_NONE) ||                 \
...
#define Z_LOG(_level, ...)                               \
	__LOG(_level, (uint16_t)LOG_CURRENT_MODULE_ID(), \
	      LOG_CURRENT_DYNAMIC_DATA_ADDR(), __VA_ARGS__)
	...
	do {                                                                   \
		if (Z_LOG_CONST_LEVEL_CHECK(_level)) {                         \
			...
			if (IS_ENABLED(CONFIG_LOG_MINIMAL)) {                  \
				...
				log_minimal_hexdump_print(                     \
					_level, (const char *)_data, _length); \
			} else if (LOG_CHECK_CTX_LVL_FILTER(is_user_context,   \
							    _level,            \
							    _filter)) {        \
				struct log_msg_ids src_level = {               \
...
#define Z_LOG_HEXDUMP(_level, _data, _length, _str)              \
	__LOG_HEXDUMP(_level, (uint16_t)LOG_CURRENT_MODULE_ID(), \
		      LOG_CURRENT_DYNAMIC_DATA_ADDR(), _data, _length, _str)
...
log_const_source_id(const struct log_source_const_data *data)
{
 ...
 */
static inline uint32_t
log_dynamic_source_id(struct log_source_dynamic_data *data)
{
...
static inline __printf_like(1, 2) void log_printf_arg_checker(const char *fmt,
							      ...)
{
	ARG_UNUSED(fmt);
}
...
#define Z_LOG_VA(_level, _str, _valist, _argnum, _strdup_action)          \
	__LOG_VA(_level, (uint16_t)LOG_CURRENT_MODULE_ID(),               \
		 LOG_CURRENT_DYNAMIC_DATA_ADDR(), _str, _valist, _argnum, \
		 _strdup_action)
	...
	do {                                                                   \
		if (Z_LOG_CONST_LEVEL_CHECK(_level)) {                         \
			...
			if (IS_ENABLED(CONFIG_LOG_MINIMAL)) {                  \
				...
				__LOG_INTERNAL_VA(is_user_context, src_level,  \
...
#define __LOG_INTERNAL_VA(is_user_context, _src_level, _str, _valist, _argnum, \
			  _strdup_action)                                      \
	do {                                                                   \
		if (is_user_context) {                                         \
			log_generic_from_user(_src_level, _str, _valist);      \
		} else if (IS_ENABLED(CONFIG_LOG_IMMEDIATE)) {                 \
			log_generic(_src_level, _str, _valist,                 \
				    _strdup_action);                           \
		} else if (_argnum == 0) {                                     \
			_LOG_INTERNAL_0(_src_level, _str);                     \
		} else {                                                       \
			uint32_t mask =                                        \
				(_strdup_action != LOG_STRDUP_SKIP) ?          \
