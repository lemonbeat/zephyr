...
#define FLASH_MIN_WRITE_SIZE DT_PROP(DT_CHOSEN(zephyr_flash), write_block_size)
...
#define SWAP_TYPE_OFFS(bank_area) \
	((bank_area)->fa_size - BOOT_MAGIC_SZ - BOOT_MAX_ALIGN * 3)
#endif

#define COPY_DONE_OFFS(bank_area) \
	((bank_area)->fa_size - BOOT_MAGIC_SZ - BOOT_MAX_ALIGN * 2)

#define IMAGE_OK_OFFS(bank_area) \
	((bank_area)->fa_size - BOOT_MAGIC_SZ - BOOT_MAX_ALIGN)
			  ...
			  size_t header_size)
{
	...
	size_t v1_min_size =
		(sizeof(uint32_t) + sizeof(struct mcuboot_img_header_v1));
...
int mcuboot_swap_type(void)
{
	...
	for (i = 0; i < BOOT_SWAP_TABLES_COUNT; i++) {
		...
		if (boot_magic_compatible_check(table->magic_primary_slot,
			...
			__ASSERT_NO_MSG(
				table->swap_type == BOOT_SWAP_TYPE_TEST ||
