...
struct ppp_peer_option_info {
	uint8_t code;
	int (*parse)(struct ppp_fsm *fsm, struct net_pkt *pkt, void *user_data);
...
#define PPP_PEER_OPTION(_code, _parse, _nack)                  \
	{                                                      \
		.code = _code, .parse = _parse, .nack = _nack, \
	}
...
#define PPP_PROTO_GET_NAME(proto_name) (ppp_protocol_handler_##proto_name)

#define PPP_PROTOCOL_REGISTER(name, proto, init_func, proto_handler,         \
			      proto_lower_up, proto_lower_down, proto_open,  \
			      proto_close)                                   \
...
#define PPP_MY_OPTION(_code, _req_add, _handle_ack, _handle_nak) \
	{                                                        \
		.code = _code, .conf_req_add = _req_add,         \
...
static inline bool ppp_my_option_is_acked(struct ppp_fsm *fsm, uint8_t code)
{
