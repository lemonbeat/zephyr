...
static int shell_cmd_connect(const struct shell *shell, size_t argc,
			     char *argv[])
{
	int err;
	bt_addr_le_t addr;
...
static int shell_cmd_scan(const struct shell *shell, size_t argc, char *argv[])
{
	struct net_if *iface = net_if_get_default();
...
static int shell_cmd_disconnect(const struct shell *shell, size_t argc,
				char *argv[])
{
	struct net_if *iface = net_if_get_default();
...
static int shell_cmd_advertise(const struct shell *shell, size_t argc,
			       char *argv[])
{
	struct net_if *iface = net_if_get_default();
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	bt_commands, SHELL_CMD(advertise, NULL, "on/off", shell_cmd_advertise),
	SHELL_CMD(connect, NULL,
		  "<address: XX:XX:XX:XX:XX:XX> <type: (public|random)>",
		  shell_cmd_connect),
	SHELL_CMD(scan, NULL, "<on/off/active/passive>", shell_cmd_scan),
	SHELL_CMD(disconnect, NULL, "", shell_cmd_disconnect),
