...
#define DNS_RESOLVER_BUF_CTR \
	(DNS_RESOLVER_MIN_BUF + CONFIG_LLMNR_RESOLVER_ADDITIONAL_BUF_CTR)
...
static void create_ipv6_addr(struct sockaddr_in6 *addr)
{
	...
	net_ipv6_addr_create(&addr->sin6_addr, 0xff02, 0, 0, 0, 0, 0, 0x01,
			     0x03);
}
...
static int bind_ctx(struct net_context *ctx, struct sockaddr *local_addr,
		    socklen_t addrlen)
{
	int ret;
	...
	if (ret < 0) {
		NET_DBG("Cannot bind to LLMNR %s port (%d)",
			local_addr->sa_family == AF_INET ? "IPv4" : "IPv6",
			ret);
		return ret;
	}
...
static int add_answer(struct net_buf *query, uint32_t ttl, uint16_t addr_len,
		      const uint8_t *addr)
{
...
static int create_answer(struct net_context *ctx, enum dns_rr_type qtype,
			 struct net_buf *query, uint16_t dns_id,
			 uint16_t addr_len, const uint8_t *addr)
{
	...
	if ((query->size - query->len) <
	    (DNS_MSG_HEADER_SIZE + (DNS_QTYPE_LEN + DNS_QCLASS_LEN) * 2 +
...
static int create_ipv4_answer(struct net_context *ctx, struct net_pkt *pkt,
			      ...
			      enum dns_rr_type qtype, struct net_buf *query,
			      uint16_t dns_id, struct sockaddr *dst,
			      socklen_t *dst_len)
{
	const uint8_t *addr;
	int addr_len;
...
#if defined(CONFIG_NET_IPV6)
		addr = get_ipv6_src(net_pkt_iface(pkt), &ip_hdr->ipv6->src);
		if (!addr) {
			return -ENOENT;
		}
...
#if defined(CONFIG_NET_IPV4)
		addr = get_ipv4_src(net_pkt_iface(pkt), &ip_hdr->ipv4->src);
		if (!addr) {
			return -ENOENT;
		}
			 ...
			 enum dns_rr_type qtype, uint16_t dns_id)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV4) && net_pkt_family(pkt) == AF_INET) {
		ret = create_ipv4_answer(ctx, pkt, ip_hdr, qtype, reply, dns_id,
					 &dst, &dst_len);
		if (ret < 0) {
			return ret;
		}
		...
		ret = create_ipv6_answer(ctx, pkt, ip_hdr, qtype, reply, dns_id,
					 &dst, &dst_len);
		if (ret < 0) {
			return ret;
		}
	...
	ret = net_context_sendto(ctx, reply->data, reply->len, &dst, dst_len,
				 NULL, K_NO_WAIT, NULL);
	if (ret < 0) {
...
static int dns_read(struct net_context *ctx, struct net_pkt *pkt,
		    union net_ip_header *ip_hdr, struct net_buf *dns_data,
