...
#define CONNMON_IP_ADDRESS_MAX \
	(CONFIG_NET_IF_MAX_IPV6_COUNT + CONFIG_NET_IF_MAX_IPV4_COUNT)
...
#define RESOURCE_INSTANCE_COUNT                                            \
	(CONNMON_MAX_ID - 4 + CONNMON_AVAIL_BEARER_MAX + CONNMON_APN_MAX + \
	 CONNMON_IP_ADDRESS_MAX + CONNMON_ROUTER_IP_ADDRESS_MAX)
...
static struct lwm2m_engine_obj_inst *connmon_create(uint16_t obj_inst_id)
{
	...
	INIT_OBJ_RES_MULTI_OPTDATA(CONNMON_IP_ADDRESSES, res, i, res_inst, j,
				   CONNMON_IP_ADDRESS_MAX, false);
...
static int lwm2m_connmon_init(const struct device *dev)
{
	...
	ret = lwm2m_create_obj_inst(LWM2M_OBJECT_CONNECTIVITY_MONITORING_ID, 0,
				    &obj_inst);
	if (ret < 0) {
