...
#define USB_ACTIVE_IF_DESC_SIZE                                     \
	(USB_PASSIVE_IF_DESC_SIZE + USB_AC_CS_IF_DESC_SIZE +        \
	 USB_FORMAT_TYPE_I_DESC_SIZE + USB_STD_AS_AD_EP_DESC_SIZE + \
...
#define FEATURE_TONE_CONTROL_SIZE \
	(FEATURE_BASS_SIZE + FEATURE_MID_SIZE + FEATURE_TREBLE_SIZE)
...
#define POS_AUTOMATIC_GAIN_CONTROL \
	(POS_GRAPHIC_EQUALIZER + FEATURE_GRAPHIC_EQUALIZER_SIZE)
#define POS_DELAY \
	(POS_AUTOMATIC_GAIN_CONTROL + FEATURE_AUTOMATIC_GAIN_CONTROL_SIZE)
#define POS_BASS_BOOST (POS_DELAY + FEATURE_DELAY_SIZE)
#define POS_LOUDNESS (POS_BASS_BOOST + FEATURE_BASS_BOOST_SIZE)

#define POS(prop, ch_idx, ch_cnt) \
	(ch_cnt * POS_##prop + (ch_idx * FEATURE_##prop##_SIZE))
...
#define CH_CFG(dev, i)                                           \
	(0x0000 | IF_USB_AUDIO_PROP(dev, i, channel_l, BIT(0)) | \
 ...
 */
#define CH_CNT(dev, i)                                 \
	(0 + IF_USB_AUDIO_PROP(dev, i, channel_l, 1) + \
...
#define FEATURES(dev, i)                                                     \
	(0x0000 | IF_USB_AUDIO_PROP(dev, i, feature_mute, BIT(0)) |          \
...
#define FEATURES_SIZE(dev, i)                                              \
	((CH_CNT(dev, i) + 1) *                                            \
	 (0x0000 +                                                         \
	  IF_USB_AUDIO_PROP(dev, i, feature_mute, FEATURE_MUTE_SIZE) +     \
	  IF_USB_AUDIO_PROP(dev, i, feature_volume, FEATURE_VOLUME_SIZE) + \
	  IF_USB_AUDIO_PROP(dev, i, feature_tone_control,                  \
	  ...
	  IF_USB_AUDIO_PROP(dev, i, feature_delay, FEATURE_DELAY_SIZE) +   \
...
#define EP_SIZE(dev, i) ((GET_RES(dev, i) / 8) * CH_CNT(dev, i) * 48)
...
#define HS_ID(i) \
	((3 * (HEADPHONES_DEVICE_COUNT + MICROPHONE_DEVICE_COUNT)) + 6 * i + 1)
...
#define INIT_IAD(iface_subclass, if_cnt)                                       \
	{                                                                      \
		...
		.bInterfaceCount = if_cnt, .bFunctionClass = AUDIO_CLASS,      \
		.bFunctionSubClass = iface_subclass, .bFunctionProtocol = 0,   \
		.iFunction = 0,                                                \
	}
...
#define INIT_STD_IF(iface_subclass, iface_num, alt_setting, eps_num)           \
	{                                                                      \
		...
		.bAlternateSetting = alt_setting, .bNumEndpoints = eps_num,    \
		.bInterfaceClass = AUDIO_CLASS,                                \
		.bInterfaceSubClass = iface_subclass, .bInterfaceProtocol = 0, \
		.iInterface = 0,                                               \
	}
...
#define INIT_CS_AC_IF(dev, i, ifaces)                                      \
	{                                                                  \
		...
		.bInCollection = ifaces, .baInterfaceNr = { 0 },           \
	}
...
#define INIT_IN_TERMINAL(dev, i, terminal_id, type)                          \
	{                                                                    \
		...
		.wTerminalType = sys_cpu_to_le16(type), .bAssocTerminal = 0, \
...
#define INIT_OUT_TERMINAL(terminal_id, source_id, type)                      \
	{                                                                    \
		...
		.bTerminalID = terminal_id,                                  \
		.wTerminalType = sys_cpu_to_le16(type), .bAssocTerminal = 0, \
		.bSourceID = source_id, .iTerminal = 0,                      \
	}
...
#define INIT_FEATURE_UNIT(dev, i, unit_id, source_id)                        \
	{                                                                    \
		...
		.bmaControls = { FEATURES(dev, i) }, .iFeature = 0,          \
	}
...
#define INIT_AS_FORMAT_I(ch_cnt, res)                                \
	{                                                            \
		...
		.bFormatType = 0x01, .bNrChannels = MAX(1, ch_cnt),  \
		.bSubframeSize = res / 8, .bBitResolution = res,     \
		.bSamFreqType = 1, .tSamFreq = { 0x80, 0xBB, 0x00 }, \
	}
...
#define INIT_STD_AS_AD_EP(dev, i, addr)                                      \
	{                                                                    \
		...
		.bInterval = 0x01, .bRefresh = 0x00, .bSynchAddress = 0x00,  \
	}
...
#define INIT_CS_AS_AD_EP                                          \
	{                                                         \
		...
		.bDescriptorSubtype = 0x01, .bmAttributes = 0x00, \
		.bLockDelayUnits = 0x00, .wLockDelay = 0,         \
	}
