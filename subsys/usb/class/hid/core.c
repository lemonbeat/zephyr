...
#define INITIALIZER_IF                                                        \
	{                                                                     \
		...
		.bInterfaceClass = HID_CLASS, .bInterfaceSubClass = 1,        \
...
#define INITIALIZER_IF                                                        \
	{                                                                     \
		...
		.bDescriptorType = USB_INTERFACE_DESC, .bInterfaceNumber = 0, \
		.bAlternateSetting = 0, .bNumEndpoints = 1,                   \
		.bInterfaceClass = HID_CLASS, .bInterfaceSubClass = 0,        \
...
#define INITIALIZER_IF_HID                                             \
	{                                                              \
		...
		.bcdHID = sys_cpu_to_le16(USB_1_1), .bCountryCode = 0, \
		.bNumDescriptors = 1,                                  \
		.subdesc[0] = {                                        \
...
#define DEFINE_HID_DESCR(x, _)                                              \
	...
	struct usb_hid_config hid_cfg_##x = {                               \
		...
		.if0_int_in_ep =                                            \
			INITIALIZER_IF_EP(AUTO_EP_IN, USB_DC_EP_INTERRUPT,  \
					  CONFIG_HID_INTERRUPT_EP_MPS),     \
		.if0_int_out_ep =                                           \
			INITIALIZER_IF_EP(AUTO_EP_OUT, USB_DC_EP_INTERRUPT, \
					  CONFIG_HID_INTERRUPT_EP_MPS),     \
	};
#else
#define DEFINE_HID_DESCR(x, _)                                             \
...
static void usb_set_hid_report_size(const struct usb_cfg_data *cfg,
				    uint16_t size)
{
...
static int hid_class_handle_req(struct usb_setup_packet *setup, int32_t *len,
				uint8_t **data)
{
	struct hid_device_info *dev_data;
	struct usb_dev_data *common;
...
static int hid_custom_handle_req(struct usb_setup_packet *setup, int32_t *len,
				 uint8_t **data)
{
		    ...
		    uint32_t max_data_len, uint32_t *ret_bytes)
{
	...
	return usb_read(cfg->endpoint[HID_INT_OUT_EP_IDX].ep_addr, data,
			max_data_len, ret_bytes);
#else
	return -ENOTSUP;
#endif
}
...
#define DEFINE_HID_DEV_DATA(x, _) struct hid_device_info usb_hid_dev_data_##x;

#define DEFINE_HID_DEVICE(x, _)                                          \
	DEVICE_AND_API_INIT(usb_hid_device_##x,                          \
			    CONFIG_USB_HID_DEVICE_NAME "_" #x,           \
			    &usb_hid_device_init, &usb_hid_dev_data_##x, \
			    &hid_config_##x, POST_KERNEL,                \
			    CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &hid_api);
