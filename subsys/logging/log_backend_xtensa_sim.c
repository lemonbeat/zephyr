...
LOG_OUTPUT_DEFINE(log_output_xsim, char_out, xtensa_log_buf,
		  sizeof(xtensa_log_buf));
...
{
	log_backend_std_sync_string(&log_output_xsim, 0, src_level, timestamp,
				    fmt, ap);
}
...
static void sync_hexdump(const struct log_backend *const backend,
			 ...
			 uint32_t length)
{
	log_backend_std_sync_hexdump(&log_output_xsim, 0, src_level, timestamp,
