...
{
#define ERR_MSG                                               \
	"argument %d in source %s log message \"%s\" missing" \
	"log_strdup()."
	uint32_t idx;
	const char *str;
	const char *msg_str;
...
void log_1(const char *str, log_arg_t arg0, struct log_msg_ids src_level)
{
	if (IS_ENABLED(CONFIG_LOG_FRONTEND)) {
...
void log_2(const char *str, log_arg_t arg0, log_arg_t arg1,
	   struct log_msg_ids src_level)
{
	if (IS_ENABLED(CONFIG_LOG_FRONTEND)) {
...
void log_3(const char *str, log_arg_t arg0, log_arg_t arg1, log_arg_t arg2,
	   struct log_msg_ids src_level)
{
	if (IS_ENABLED(CONFIG_LOG_FRONTEND)) {
		 ...
		 struct log_msg_ids src_level)
{
	if (IS_ENABLED(CONFIG_LOG_FRONTEND)) {
		...
		struct log_msg *msg = log_msg_hexdump_create(
			str, (const uint8_t *)data, length);
...
void log_core_init(void)
{
	...
	if (IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING)) {
		for (int i = 0; i < log_sources_count(); i++) {
			...
			LOG_FILTER_SLOT_SET(filters, LOG_FILTER_AGGR_SLOT_IDX,
					    level);
		}
	}
}
...
static void thread_set(k_tid_t process_tid)
{
	...
	if (CONFIG_LOG_PROCESS_TRIGGER_THRESHOLD && process_tid &&
			     ...
			     struct log_msg *msg)
{
	if (IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING)) {
		...
		backend_level =
			log_filter_get(backend, log_msg_domain_id_get(msg),
				       log_msg_source_id_get(msg),
	...
	if (IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING)) {
		...
		if (backend == NULL) {
			...
			for (int i = 0; i < log_backend_count_get(); i++) {
				...
				current = log_filter_set(
					iter_backend, domain_id, src_id, level);
				max = MAX(current, max);
			}
			...
			LOG_FILTER_SLOT_SET(filters,
					    log_backend_id_get(backend), level);
			       ...
			       uint32_t level)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG((level <= LOG_LEVEL_DBG) &&
...
{
	return IS_ENABLED(CONFIG_LOG_STRDUP_POOL_PROFILING) ? log_strdup_max :
								    0;
}
...
{
	struct log_strdup_buf *dup =
		CONTAINER_OF(str, struct log_strdup_buf, buf);
...
void z_vrfy_z_log_string_from_user(uint32_t src_level_val, const char *str)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG((IS_ENABLED(CONFIG_LOG_PRINTK) ||
				     (level >= LOG_LEVEL_ERR)) &&
					    (level <= LOG_LEVEL_DBG),
				    "Invalid log level"));
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(domain_id == CONFIG_LOG_DOMAIN_ID,
...
void z_impl_z_log_hexdump_from_user(uint32_t src_level_val,
...
void z_vrfy_z_log_hexdump_from_user(uint32_t src_level_val,
				    const char *metadata, const uint8_t *data,
				    uint32_t len)
{
	union {
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(
		...
		"Invalid log level"));
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(src_level_union.structure.domain_id ==
					    CONFIG_LOG_DOMAIN_ID,
				    "Invalid log domain_id"));
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(src_level_union.structure.source_id <
					    log_sources_count(),
				    "Invalid log source id"));

	if (IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING) &&
	    (src_level_union.structure.level >
	     LOG_FILTER_SLOT_GET(log_dynamic_filters_get(
	...
	if (IS_ENABLED(CONFIG_LOG_IMMEDIATE)) {
		log_hexdump_sync(src_level_union.structure, metadata, data,
				 len);
	} else {
