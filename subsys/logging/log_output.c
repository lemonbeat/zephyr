...
static const char *const severity[] = { NULL, "err", "wrn", "inf", "dbg" };

static const char *const colors[] = {
	NULL, LOG_COLOR_CODE_RED, /* err */
...
extern void z_vprintk(out_func_t out, void *log_output, const char *fmt,
		      va_list ap);
...
static int print_formatted(const struct log_output *log_output, const char *fmt,
			   ...)
{
	va_list args;
	int length = 0;
...
static int timestamp_print(const struct log_output *log_output, uint32_t flags,
			   uint32_t timestamp)
{
	int length;
	bool format = (flags & LOG_OUTPUT_FLAG_FORMAT_TIMESTAMP) |
	...
	if (!format) {
		...
		if (IS_ENABLED(CONFIG_LOG_BACKEND_NET) &&
			...
			length = print_formatted(
				log_output, "1970-01-01T%02d:%02d:%02d.%06dZ ",
...
static void color_print(const struct log_output *log_output, bool color,
			bool start, uint32_t level)
{
	if (color) {
...
static void color_prefix(const struct log_output *log_output, bool color,
			 uint32_t level)
{
	color_print(log_output, color, true, level);
}

static void color_postfix(const struct log_output *log_output, bool color,
			  uint32_t level)
{
...
static int ids_print(const struct log_output *log_output, bool level_on,
		     bool func_on, uint32_t domain_id, uint32_t source_id,
		     uint32_t level)
{
	int total = 0;
	...
	total += print_formatted(
		log_output,
		(func_on && ((1 << level) & LOG_FUNCTION_PREFIX_MASK)) ? "%s." :
									       "%s: ",
...
static void std_print(struct log_msg *msg, const struct log_output *log_output)
{
...
static uint32_t prefix_print(const struct log_output *log_output,
			     uint32_t flags, bool func_on, uint32_t timestamp,
			     uint8_t level, uint8_t domain_id,
			     uint16_t source_id)
{
	uint32_t length = 0U;
	...
	length += ids_print(log_output, level_on, func_on, domain_id, source_id,
			    level);

	return length;
}

static void postfix_print(const struct log_output *log_output, uint32_t flags,
			  uint8_t level)
{
	color_postfix(log_output, (flags & LOG_OUTPUT_FLAG_COLORS), level);
	newline_print(log_output, flags);
}
			    ...
			    struct log_msg *msg, uint32_t flags)
{
	...
	prefix_offset = raw_string ? 0 :
					   prefix_print(log_output, flags, std_msg,
						  timestamp, level, domain_id,
						  source_id);
		       ...
		       const char *fmt, va_list ap, uint32_t flags)
{
	...
	if (IS_ENABLED(CONFIG_LOG_MIPI_SYST_ENABLE) &&
	    flags & LOG_OUTPUT_FLAG_FORMAT_SYST) {
		log_output_string_syst_process(log_output, src_level, fmt, ap,
					       flags);
		return;
	}

	if (!raw_string) {
		prefix_print(log_output, flags, true, timestamp, level,
			     domain_id, source_id);
	}
			...
			uint32_t length, uint32_t flags)
{
	...
	if (IS_ENABLED(CONFIG_LOG_MIPI_SYST_ENABLE) &&
	    flags & LOG_OUTPUT_FLAG_FORMAT_SYST) {
		log_output_hexdump_syst_process(log_output, src_level, data,
						length, flags);
		return;
	}

	prefix_offset = prefix_print(log_output, flags, true, timestamp, level,
				     domain_id, source_id);
	...
	while (length) {
		...
		hexdump_line_print(log_output, data, part_len, prefix_offset,
				   flags);
