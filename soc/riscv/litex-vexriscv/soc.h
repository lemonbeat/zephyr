...
{
	return (sys_read8(addr) << 24) | (sys_read8(addr + 0x4) << 16) |
	       (sys_read8(addr + 0x8) << 8) | sys_read8(addr + 0xc);
}
...
static inline void litex_write(volatile uint32_t *reg, uint32_t reg_size,
			       uint32_t val)
{
	...
	for (i = 0; i < reg_size; ++i) {
		shifted_data = val >>
			       ((reg_size - i - 1) * LITEX_SUBREG_SIZE_BIT);
